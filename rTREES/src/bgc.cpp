...
BiogeochemicalCycles::BiogeochemicalCycles(trees_params& treesParams)
{
  ...
  // verbosity=treesParams.verbosity;
  ...
  if (treesParams.useLeafModule == 1)
  {
    ...
    Narray.resize(500);
    rarray.resize(500);
    SingleLeafBiomassCarbon.resize(500);
    SingleLeafBiomassNitrogen.resize(500);
    SingleLeafNSC.resize(500);
    SingleLeafchloroplastStarch.resize(500);
    SingleLeafchloroplastSugar.resize(500);
    SingleLeafStoredNitrogen.resize(500);
    SingleLeafRubiscoNitrogen.resize(500);
    SingleLeafGrowthRespiration.resize(500);
    SingleLeafArea.resize(500);
    SingleLeafAreaPotential.resize(500);
    SingleLeafThermTm.resize(500);
    for (int j = 0; j < 500; j++)
    {
    ...
    // leaf initialization
    double shoot_init, leaf_init, stem_init, projGNDarea;

    projGNDarea = calc_projGndArea(treesParams, Karray);
    // if(verbosity>0){
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "projected canopy ground area = " << projGNDarea << "\n";
    // }
    // g/m2 = kg/ha * 1/10
                  // 1/(SLA*0.0001)*1/10* projectedArea_init*0.0001=g
    shoot_init = 1.0/(treesParams.SLA_max*10.0)*treesParams.projectedArea_init; // in grams C
    leaf_init = 0.8*g_to_kg(shoot_init)/cm2_to_ha(projGNDarea) ; // kgC/ha
    stem_init = 0.2*g_to_kg(shoot_init)/cm2_to_ha(projGNDarea) ; // kgC/ha

    // leaf carbon
    leafBiomassCarbon = leaf_init/(1.0+treesParams.leafNSCscalar);//!!!
    // if(verbosity>0){
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "leafBiomassCarbon = " << leafBiomassCarbon << "\n";
    // }
    leafNSC = leafBiomassCarbon*treesParams.leafNSCscalar; //retained
    ...
    // leafN= leafBiomassCarbon / treesParams.leaf_minCN;//rTREES consolidated with definitive TREES.
    leafN = leafBiomassCarbon / (0.5 * (treesParams.leaf_minCN + treesParams.leaf_maxCN));//rTREES consolidated with definitive TREES.

    leafBiomassNitrogen = leafN;
    // if(verbosity>0){
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "leafBiomassNitrogen = " << leafBiomassNitrogen << "\n";
    // }
    CN = leafBiomassCarbon/leafBiomassNitrogen;
    // rubisco N, chloroplast storage, and stored N eqns are retained
    leafRubiscoNitrogen = leafBiomassNitrogen * treesParams.Nrubisco;

    chloroplastStarch = 0.01*leafRubiscoNitrogen*leafBiomassCarbon/leafBiomassNitrogen;
    chloroplastSugar = 0.0;

    //leafStoredNitrogen[0] = leafNSC[0]/CN;
    leafStoredNitrogen = 0.0;

    // stem initialization; very young stem tissue --> keep same ratios as leaf tissue
    liveStemCarbon = stem_init/(1+ treesParams.leafNSCscalar*0.4) ;
    stemNSC = treesParams.leafNSCscalar*0.4 * liveStemCarbon;
    liveStemNitrogen = 0.003 * liveStemCarbon;

    // negligible dead stem tissue
    deadStemCarbon = 0.000001;
    deadStemNitrogen = 0.000001;

    // leaf and stem residues set to close to 0
    leafResidueCarbon = 0.000001;
    leafResidueNitrogen = 0.000001;
    stemResidueCarbon = 0.000001;
    stemResidueNitrogen = 0.000001;
  ...
  {
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"_bigleaf \n";
    ...
    {
      // if(treesParams.version==1){
      //   leafResidueCarbon = (1.0/treesParams.leafLifeSpan)*treesParams.lai / treesParams.SLA * 10000.0 * SLAscalar;
      //   leafResidueNitrogen = (0.5 * treesParams.Nleaf * treesParams.lai * 10000.0)*SLAscalar;
      // }
      // else{
        leafResidueCarbon = treesParams.Cbelowground * treesParams.Clitter_frac * 0.5;
        leafResidueNitrogen = leafResidueCarbon / (64.0 * treesParams.leafLifeSpan+0.001);
      // }

      //set stem residue to 5% of standing stem biomass
      //stem [N] is assumed to be low (0.3% of carbon)
      // if(treesParams.version==1){
      //   stemResidueCarbon = 0.05 * treesParams.Cstem;
      // }
      // else{
        stemResidueCarbon = treesParams.Cbelowground * treesParams.Clitter_frac * 0.5;
      // }

      stemResidueNitrogen = 0.003 * stemResidueCarbon;
      if (treesParams.leafLifeSpan > 1.0)
      {
        leafBiomassCarbon = (1.0 - 1.0/treesParams.leafLifeSpan)*treesParams.lai/treesParams.SLA*10000.0+0.01;
        // if(treesParams.version==1){
        //
        // }
        // else{
          //DSM July 2024
          leafNSC = leafBiomassCarbon * treesParams.leafNSCscalar;
        // }

      }
      else
      {
        leafBiomassCarbon = (1.0 - 0.9/treesParams.leafLifeSpan)*treesParams.lai/treesParams.SLA*10000.0+0.01;
        // if(treesParams.version==1){
        //
        // }
        // else{
          //DSM July 2024
          leafNSC = 10.0*leafBiomassCarbon ;
          // std::cout << "leafNSC = " << leafNSC[0] << endl;
        // }
      }
    }
    else //best for annual plants
    {
      leafBiomassCarbon = treesParams.lai/treesParams.SLA*10000.0+0.01;

      leafNSC  = leafBiomassCarbon  * treesParams.leafNSCscalar;

      liveStemCarbon = treesParams.Csapwood;
      stemNSC = treesParams.leafNSCscalar*0.4 * liveStemCarbon;
      liveStemNitrogen = 0.003 * liveStemCarbon;
      deadStemCarbon = treesParams.Cstem;
      deadStemNitrogen = 0.003 * deadStemCarbon;
      leafResidueCarbon = 0.00001;
      leafResidueNitrogen = 0.000001;
      stemResidueCarbon = 0.00001;
      stemResidueNitrogen = 0.000001;
    }
    ...
    leafN = leafBiomassCarbon / (0.5 * (treesParams.leaf_minCN + treesParams.leaf_maxCN));
    leafBiomassNitrogen = leafN;
    //rTREES THis differnce does not exist in all trees versions by same name^^
    ...
    CN = leafBiomassCarbon/leafBiomassNitrogen;
    // if(verbosity>0){
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "leafBiomassCarbon = " << leafBiomassCarbon << "; " << leafBiomassNitrogen << "; CN = " << CN << "\n";
    // }
    leafStoredNitrogen = leafNSC/treesParams.leaf_maxCN;
  }
  ...
  // heterotrophicRespiration.resize(nRoots);
  heterotrophicRespiration=std::vector<double>(nRoots);

  maximumRootBiomassCarbon=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  fineRootBiomassCarbon=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  fineRootBiomassNitrogen=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  coarseRootBiomassCarbon=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  coarseRootBiomassNitrogen=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rhizosphereCl=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rhizosphereNl=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rootResidueCarbon=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rootResidueNitrogen=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rhizosphereLiveMicrobialCarbon=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rhizosphereDeadMicrobialCarbon=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));

  rhizosphereMicrobialNitrogen=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rhizosphereMineralNitrogen=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rhizosphereAmmoniumNitrogen=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rhizosphereNitrateNitrogen=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rootNSC=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rhizosphereLabileCarbon=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rootMineralNitrogen=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rootExudateSugarCarbon=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rootExudateAminoAcidCarbon=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  rootExudateAminoAcidNitrogen=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));

  humusCarbon=std::vector<double>(nRoots);
  humusNitrogen=std::vector<double>(nRoots);
  soilAmmoniumNitrogen=std::vector<double>(nRoots);
  soilNitrateNitrogen=std::vector<double>(nRoots);
  nitrogenLeaching=std::vector<double>(nRoots);
  rootArea=std::vector<std::vector<double>> (nRoots,std::vector<double>(nRootOrders));
  rootAreaScalar=std::vector<double>(nRoots);

  dCrootResidue=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  dNrootResidue=std::vector<std::vector<double>>(nRoots,std::vector<double>(nRootOrders));
  ...
  for (int j = 0; j < nRoots; j++)
  {
    ...
    for (int k = 0; k < nFineRootOrders; k++)
    {
      ...
      fineRootBiomassNitrogen[j][k] = fineRootBiomassCarbon[j][k] /
        (leafBiomassCarbon/leafBiomassNitrogen*rootScalar);
      ...
      if (j < 3)
      {
        rhizosphereCl[j][k] += 0.10/3.0*treesParams.Clitter_frac * treesParams.Cbelowground;
      }
      ...
      //FLAG these may be best made into parameters or just moved to leaf setup
      if(treesParams.useLeafModule==1){
        ...
        rhizosphereLiveMicrobialCarbon[j][k] = 0.012*rhizosphereCl[j][k];
        rhizosphereDeadMicrobialCarbon[j][k] = 0.02*rhizosphereLiveMicrobialCarbon[j][k];
      }
    ...
    for (int k = nFineRootOrders; k < nRootOrders; k++)
    {
      ...
      //FLAG try with this on and off using FP
      // if(treesParams.version==1||treesParams.version==3){
      ...
      {
        rhizosphereCl[j][k] = 0.0001;
        rootResidueCarbon[j][k] = 0.001*scalar*coarseRootBiomassCarbon[j][k];
        rootResidueNitrogen[j][k] = 0.001*scalar*coarseRootBiomassNitrogen[j][k];
      }
      sumCl += rhizosphereCl[j][k];
      if (j < 3)
      {
      ...
      //FLAG See previous flag for making this parameters
      if(treesParams.useLeafModule==1){
        rhizosphereLiveMicrobialCarbon[j][k] = 0.12 * rhizosphereCl[j][k];
        rhizosphereDeadMicrobialCarbon[j][k] = 0.2*rhizosphereLiveMicrobialCarbon[j][k];
      }
      ...
      rhizosphereLabileCarbon[j][k] = 0.01*rootNSC[j][k];
      rootMineralNitrogen[j][k] = 0.1*fineRootBiomassNitrogen[j][k];
      //allow for a scaling of the initial nutrient levels in the microbiome
    ...
    //FLAG very important for microbial influence on tested leaf module simu.
    if(treesParams.useLeafModule==1){
      ...
      {
        rhizosphereLiveMicrobialCarbon[j][k] += 0.001*humusCarbon[j];
        rhizosphereDeadMicrobialCarbon[j][k] += 0.2*0.001*humusCarbon[j];
        rhizosphereMicrobialNitrogen[j][k] += 0.001*0.125*humusCarbon[j];;
      }
    }

    //State variables for mineral nitrogen in the bulk soil
    soilAmmoniumNitrogen[j] = 0.1*humusNitrogen[j];//PROP
    soilNitrateNitrogen[j] = 0.9*humusNitrogen[j];//PROP
    //Flux variable for nitrogen leaching set to zero
    nitrogenLeaching[j] = 0.0;//PROP
  ...
  plantNstatus = 1.0; //values are between 0 and 1
  //keep a record of the input lateral root Weibull curve parameters
  lat_Root_b_value_init = treesParams.lat_Root_b_value;
  lat_Root_c_value_init = treesParams.lat_Root_c_value;
...
BiogeochemicalCycles::~BiogeochemicalCycles()
	{
  // if(verbosity>1){
	// /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "Deconstructing bgc does nothing" << "\n";
    // }
	// clearPools();
}
...
//
double BiogeochemicalCycles::C4photosynthesis(trees_params &treesParams,//rTREES made by ref 20221020
						double Ca,
						double Iin,
						double phiJ,
						double Tl,
						...
						double& Cm)
{
	...
	Vcmax = Vcmax25 * exp((Tl-25.0)*E_Vcmax*1000.0/(298.1*R_gas*Tlk));
	Vpmax = Vpmax25 * exp((Tl-25.0)*E_Vpmax*1000.0/(298.1*R_gas*Tlk));
	Kp = Kp25 * exp((Tl-25.0)*E_Kp*1000.0/(298.1*R_gas*Tlk));
	Jmax = Jmax25 * exp((Tl-25.0)*E_Jmax*1000.0/(298.1*R_gas*Tlk));
...
//
void BiogeochemicalCycles::coupledA4_gc(trees_params &treesParams,//rTREES made by ref 20221020
                                          double Ca,
                                          double Iin,
					  double phiJ,
                                          double Tl,
...
//
double BiogeochemicalCycles::C3photosynthesis(
    trees_params &treesParams,//rTREES made by ref 20221020
    double Ca,
		double Pa,
		double Iin,
		double phiJ,
...
)
{
	...
	Tlk = Tl + 273.1;              //absolute temperature
	Vcmax = Vcmax25 * exp((Tlk-298.0)*E_Vcmax*1000.0/(298.0*R_gas*Tlk));
	Kc = Kc25 * exp((Tlk-298.0)*E_Kc*1000.0/(298.0*R_gas*Tlk));
	Ko = Ko25 * exp((Tlk-298.0)*E_Ko*1000.0/(298.0*R_gas*Tlk));
	Rd = Rd25 * exp((Tlk-298.0)*E_Rd*1000.0/(298.0*R_gas*Tlk));
	// Rm = 0.5 * Rd;                 //mesophyll mitochondrial respiration, umol m-2 s-1
	gammaStar = gammaStar25 * exp((Tlk-298.0)*E_gammaStar*1000.0/(298.0*R_gas*Tlk));
	Jmax = Jmax25 * exp((Tlk-298.0)*E_Jmax*1000.0/(298.0*R_gas*Tlk));
...
//
void BiogeochemicalCycles::coupledA3_gc(
    trees_params &treesParams,//rTREES made by ref 20221020
    double Ca,
    double Pa,
    double Iin,
...
//SC RL 1613 $ 3.2.3 @ 20240630
//
//This function computes carbon maximization with modification as xylem cavitation occurs
//  Implemented by DSM on 5/25/2024
//  where
//        xylemCost = (kcmax - kc) / (kcmax - kcrit) (Ref: Sperry et al., 2017 PC&E)
//        gc0 is proposed from the hydraulics
//
//  Solution: Find gc such that An(gc) = (1-xylemCost) x A_ref(gc0)
//              - An is monotonically increasing and asymptotic with gc
//              - a binary search is used to find gc
//
...
{
  bool stop;
  double A_ref, gc_min, gc_max, xylemCost;
  gc_min = 0.00125;
  gc_max = gc0;


  //
  //Calculate the xylem cost of supporting photosynthesis
  //where kcmax is maximum canopy hydraulic conductance at the current amount of cavitation
  //and kc is the current canopy hydraulic conductance
  //Note: This function steepens with progressive cavitation
  //Source: Sperry, J.S., M. Venturas, W.R.L. Anderegg, M. Mencuccini, D.S. Mackay, Y. Wang, D. Love. 2017.
  //        Predicting stomatal response to the environment from the optimization of photosynthetic gain and
  //        hydraulic cost. Plant, Cell & Environment, 40(6), 816-830, DOI: 10.1111/pce.12852.
  //

  kcrit = 0.0005*kcmax;
  xylemCost = (kcmax - kc) / (kcmax - kcrit);
  if (xylemCost < 0.005) xylemCost = 0.005;

  stop = false;
  //
  //Calculate reference photosynthesis at the proposal stomatal conductance
  //If reference photosynthesis is zero or negative, then set gc to minimum
  //
  A_ref = C3photosynthesis(treesParams, Ca, Pa, Iin, phiJ, Tl, 1.005*gc_max, Nmax, Ci, Cc, NG, NS);
  if (A_ref <= 0.0)
  {
    An = A_ref;
    gc = gc_min;
  ...
  //if (xylemCost > 0.1 && A_ref > 0.0)
  //std::cout << "high xylemCost = " << xylemCost << "; Iin = " << Iin << "; gc0 = " << gc0;
  //
  //Binary search algorithm to find gc
  //
  while (stop == false)
  {
    gc = 0.5 * (gc_min + gc_max);
    An = C3photosynthesis(treesParams, Ca, Pa, Iin, phiJ, Tl, gc, Nmax, Ci, Cc, NG, NS);
    if (An > (1.0-xylemCost)*A_ref)
    {
      gc_max = gc; //solution can be no higher than current gc
    }
    else
    {
      gc_min = gc; //solution can be low lower than the current gc
    }
    if ((gc_min/gc_max) > 0.9999 && (gc_min/gc_max) < 1.0001)
    {
      stop = true; // solution found
  ...
  //if (xylemCost > 0.1 && A_ref > 0.0)
  //std::cout << "; gc = " << gc << "; A_ref = " << A_ref << "; An = " << An << endl;
}//EC RL 1698 $ 3.2.3 @ 20240630
...
void BiogeochemicalCycles::storeGlycineAndSerine(double NGsun,
						 ...
						 double Lshd)
{
	glycineNitrogen += (NGsun * Lsun + NGshd * Lshd)* 0.2521206;
	glycineCarbon += (NGsun * Lsun + NGshd * Lshd) * 0.4323852;
	serineNitrogen += (NSsun * Lsun + NSshd * Lshd) * 0.2521206;
	serineCarbon += (NSsun * Lsun + NSshd * Lshd) * 0.6485778;
///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << glycineNitrogen[0] << '\t' << glycineCarbon[0] << '\t' << serineNitrogen[0] << '\t' << serineCarbon[0] << "\n";
}
...
//
//     August 2023 DK - Added water-modulated effective quantum yield of Photosystem II (phi2; TREES_model_3.2.1_ChlF)
//
double BiogeochemicalCycles::photosynthesis(
    trees_params &treesParams,//rTREES made by ref 20221020
    double Jmax_mult,
    double thetaJ,
    double phiJ,
...
)
{
	...
	double betaA;                   // (m2s/umol) exponential decline rate of phi2 with increasing light
	double phi2;                    // (fraction) effective quantum yield of PSII
	double phi2_hyd;                // (fraction) phi2 under a reference light
	double aa,bb,cc,det;
	...
	g = gin = in.g*1.0e6/(R_gas*tk);    //convert conductance from m/s -. umol/m2/s/Pa
	if ( g < 0.00000001 )
	{
	  g = 0.00000001;
	}
	...
	Jmax = Jmax25*exp((tk-298.0)*Ea/(R_gas*tk*298.0))*
	  (1.0+exp((S*298.0-H)/(R_gas*298.0)))/(1.0+exp((S*tk-H)/(R_gas*tk)));
	...
	//Ji = in.irad * phiJ;
	//DK - August 2023
	//added beta function to implement  PSII photochemistry (Pleban et al., 2020)
	if (treesParams.usePhiPS2 == 0)
	{
	...
	{
	  phi2_hyd = treesParams.beta0_refQ + (treesParams.beta1_refQ - treesParams.beta0_refQ) * exp(-pow(-leafpsi/treesParams.beta2_refQ,treesParams.beta3_refQ));
	  betaA = log((phi2_hyd - treesParams.kappa_PS2)/(treesParams.alpha_PS2 - treesParams.kappa_PS2))/treesParams.Q_ref;
	  phi2 = (treesParams.alpha_PS2 - treesParams.kappa_PS2) * exp(betaA*in.irad) + treesParams.kappa_PS2;
	  J = in.irad * phi2 * 0.31 * 2.0;
	}
	...
	if (A >= 0.0)
	{
	  g = (A)/(Ca-Ci+0.01); //g is umol/m2/s/Pa =(umol/m2/s)/(Pa)//AAA
	  out.g = g * (R_gas * tk) / 1.0e6;
	}
	...
	//Cuticular conductance //moved to
	if (out.g < 0.01*r_sun*treesParams.Gsref0/1.6/42.0)
	{
	  out.g = 0.01*r_sun*treesParams.Gsref0/1.6/42.0;
	  // out.g=0.0000001;
	  // out.g = g;
	}
	D = in.D;
	...
	out.betaA = betaA;              //(m2s/umol) exponential decline rate of phi2 with increasing light
	out.phi2 = phi2;                //(fraction) effective quantum yield of PS2
	return out.A;
...
//
double BiogeochemicalCycles::computeRootArea(trees_params &treesParams)//rTREES made by ref 20221020
{
...
//SRL1 is defined at a root diameter of 0.00025 m and root length (m) per gram of carbon
...
double BiogeochemicalCycles::computeRootArea(
    trees_params &treesParams,//rTREES made by ref 20221020
    double SRA,
    int j,
    int k
...
//		Meinen et al 2009 Oecologia
//
double BiogeochemicalCycles::computeFineRootArea(trees_params &treesParams)//rTREES made by ref 20221020
{
        double totalRootArea;
...
//
void BiogeochemicalCycles::computeMaximumRootBiomassCarbon(trees_params &treesParams)//rTREES made by ref 20221020
{
	// double maxCarbon,
	double totalRootAxialLength, rootAxialLength, lateralLength,lateralLength0, porosity;
	...
	lateralLength0=treesParams.drlat[3]*totalRootAxialLength+0.0000001;
...
//how many defined cylinders fit into a hectare, m2 m-2
		areaScalar = 10000.0/(M_PI*pow(lateralLength0, 2.0));
		rootAreaScalar[j] = 1.0/areaScalar;
//DSM June 2023
		porosity = treesParams.layer_porosity[j+3];
		...
		for (int k = 0; k < nRootOrders; ++k)
		{
			...
			maximumRootBiomassCarbon[j][k] = areaScalar*allocationScalar*maxRootLength/SRL+0.01;
        		SRL /= pow(treesParams.rootDiamMultiplier, 2.0);
        		rootDiam *= treesParams.rootDiamMultiplier;
...
//
double BiogeochemicalCycles::computeCanopyCover(trees_params &treesParams)//rTREES made by ref 20221020
{
	double totalRootAxialLength,  maximumLateralLength;//rootAxialLength,
...
//Define the canopy closure based on crown diameter
        if (treesParams.usePhenology == false)
        {
                ...
                // if(verbosity>0){
/*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "canopy_cover = " << canopy_cover << "\n";
                  // }
        }
        else
        {
...
void BiogeochemicalCycles::computeLateralRootWeibulls(trees_params& treesParams)
{
        ...
        {
        	bsum = csum = 0.0;
                bval = lat_Root_b_value_init;
                cval = lat_Root_c_value_init;
...
{
        double bval, cval;
        double  CN, CNratio, SLAscalar, rootScalar;//totalRootArea,
	...
	CNratio = MIN(1.2, CNratio);
        bval = lat_Root_b_value_init*CNratio;
        cval = lat_Root_c_value_init/CNratio;
	rootCarbonRatio = fineRootBiomassCarbon[j][2]/totalRootCarbon;
        bsum += bval*rootCarbonRatio;
        csum += cval*rootCarbonRatio;
        for (int k = 1; k >= 0; k--)
		...
		CNratio = MAX(0.8, CNratio);
		CNratio = MIN(1.2, CNratio);
        	bval = lat_Root_b_value_init*(bfactor*CNratio);
                cval = lat_Root_c_value_init*(cfactor/CNratio);
		rootCarbonRatio = fineRootBiomassCarbon[j][k]/totalRootCarbon;
                bsum += bval*rootCarbonRatio;
                csum += cval*rootCarbonRatio;
		bfactor *= 0.95;
	...
	bfactor = 1.0/0.95;
	cfactor = 0.95;
        bval = lat_Root_b_value_init*bfactor;
        cval = lat_Root_c_value_init*cfactor;
        for (int k = 3; k <= 4; k++)
        {
		rootScalar *= 1.25;
		CN = fineRootBiomassCarbon[j][k]/fineRootBiomassNitrogen[j][k];
...
//
void BiogeochemicalCycles::computeNSCfluxes(trees_params &treesParams,//rTREES made by ref 20221020
					    double kratio,
					    std::vector<double> &kratio_vector)
{
...
//
void BiogeochemicalCycles::computeNfluxes(trees_params &treesParams,//rTREES made by ref 20221020
					  double kratio,
					  std::vector<double> &kratio_vector)
{
...
//
void BiogeochemicalCycles::computeRootExudates(trees_params &treesParams,//rTREES made by ref 20221020
                                               std::vector<double> &thetaSoil,
					       std::vector<double> &kratio_vector)
{
	for (int j = 0; j < nRoots; j++)
        {
...
*/
		...
		if (reproduction / CN > 0.95 * leafStoredNitrogen)
		{
			reproduction = CN * 0.95 * leafStoredNitrogen;
		}
		if (reproduction < 0.0)
		{
			reproduction = 0.0;
		}
		putChloroplastStarch(getChloroplastStarch() - 0.17*reproduction);
		fruitCarbon += reproduction;
		fruitNitrogen += reproduction / CN;
		fromStem = getStemNSC()/nsc;
		stemNSC -= fromStem*reproduction;
		leafNSC -= (1.0-fromStem)*reproduction;
		leafStoredNitrogen -= reproduction / CN;
...
//
void BiogeochemicalCycles::updateLeafCarbonNitrogenPools(
    int k,
    ...
    double& N_pos_demand)
{
  double CN, nsc, nsc_stem,nsc_stem_frac,delta_nsc, deltaBiomassC, deltaBiomassN, GRcost, SLA_instant;
  double total_N_demand, residual_N_demand, NfromStorage;
  double N_avail_rate_plant, projGNDarea;

  //projGNDarea = treesParams.pot_size; //scale to ground surface area of pot
  ...
  if (deltaAreaL > 0.0)
  {
    N_avail_rate_plant = plantNstatus;
    SLA_instant = calcSLA(plantNstatus, treesParams.SLA_max, treesParams.SLA_min);
    CN = treesParams.leaf_maxCN - (treesParams.leaf_maxCN-treesParams.leaf_minCN)*N_avail_rate_plant;
    nsc = getLeafNSC();
    projGNDarea = calc_projGndArea(treesParams, Karray);

    //kgC ha-1 = cm2 cm-2 * kgC m-2 m2 ha-1
    //RL was too limiting for Brassica steep R-shaped VC
    delta_nsc = deltaAreaL / projGNDarea/ SLA_instant * 10000.0 * 1.17;

    //If NSC is limiting
    nsc_stem_frac = nsc_stem = 0.0;
    if (delta_nsc > 0.99*nsc)
    {
      nsc_stem = delta_nsc - 0.99*nsc;
      if (nsc_stem > 0.99*getStemNSC())
      {
        nsc_stem = 0.99*getStemNSC();
      }
      nsc_stem_frac = nsc_stem / delta_nsc;
      delta_nsc = 0.99*nsc+nsc_stem;


    }
    //Growth respiration cost, GRcost
    GRcost = delta_nsc * 0.17 / 1.17;
    total_N_demand = (delta_nsc-GRcost) / CN;
    //If leaf nitrogen is limiting
    if (total_N_demand > 0.99*leafStoredNitrogen)
    {
      NfromStorage = 0.99*leafStoredNitrogen;
      delta_nsc = NfromStorage * CN * 1.17;
      GRcost = delta_nsc * 0.17 / 1.17;
    }
    //Use total nitrogen desired if unlimiting to set the uptake demand
    ...
    treesParams.SLA = (treesParams.SLA * leafBiomassCarbon + SLA_instant * deltaBiomassC) / (leafBiomassCarbon + deltaBiomassC);
    leafBiomassCarbon += deltaBiomassC;
    SingleLeafBiomassCarbon[k] += deltaBiomassC; // structural C, in kgC
    leafNSC  -= delta_nsc * (1.0 - nsc_stem_frac);
    stemNSC  -= nsc_stem_frac * delta_nsc;



    //Update leaf and whole canopy nitrogen
    deltaBiomassN = deltaBiomassC / CN;
    leafBiomassNitrogen += deltaBiomassN;
    SingleLeafBiomassNitrogen[k] += deltaBiomassN; // structural N, in kgC
    leafStoredNitrogen -= deltaBiomassN;

    //RL should be used here, as it increases leaf area per unit carbon consumed
    deltaAreaL = deltaBiomassC / 10000.0 * SLA_instant * projGNDarea;
...
)
{
  ...
  double total_N_demand, residual_N_demand, NfromStorage;
  double N_avail_rate_plant = plantNstatus;
  ...
  if (delta_lai > 0.0)
  {
    ...
    delta_nsc = RL * delta_lai / SLA_instant * 10000.0 * 1.17;
    ...
    treesParams.SLA = (treesParams.SLA * leafBiomassCarbon + SLA_instant * (delta_nsc-GRcost)) / (leafBiomassCarbon + delta_nsc-GRcost);
    leafBiomassCarbon += (delta_nsc-GRcost);
    leafNSC -= delta_nsc;
    leafBiomassNitrogen += (delta_nsc-GRcost)/CN;
    leafStoredNitrogen -= (delta_nsc-GRcost)/CN;

  }
  //when leaves are senescing and leaf carbon exceeds the leaf bud carbon target
  else if (delta_lai < 0.0 && leafBiomassCarbon > minLeafC)
  {
    delta_nsc = delta_lai / treesParams.SLA * 10000.0;//rTREES watch!
    CN = leafBiomassCarbon/leafBiomassNitrogen;
    leafBiomassCarbon += delta_nsc;
    //retain biomass C and N for next budburst
    if (leafBiomassCarbon < minLeafC)
    {
      delta_nsc += (minLeafC-leafBiomassCarbon);
      leafBiomassCarbon = minLeafC;
    }
    //0.5 denotes 50% retranslocation
    //apportioned to leaf bud and reserves
    leafBiomassNitrogen += 0.5*delta_nsc/CN;
    leafBiomassNitrogen += 0.25/baseLAI*delta_nsc/CN;
    leafStoredNitrogen -= (0.5-0.25/baseLAI)*delta_nsc/CN;
    leafResidueCarbon -= delta_nsc;
    leafResidueNitrogen -= 0.5*delta_nsc/CN;
  }
}
...
//
void BiogeochemicalCycles::computeLeafNdemand(
    trees_params &treesParams,//rTREES made by ref 20221020
		double& delta_lai,
		double N_neg_fract,
		double& N_neg_demand,
...
//
void BiogeochemicalCycles::updateStemCarbonNitrogenPools(
    trees_params &treesParams,//rTREES made by ref 20221020
    double N_avail_rate_plant,
    double kratio,
    double nscRatio,
...
//Live stem wood mortality
	CN = liveStemCarbon/liveStemNitrogen;
	CN = MAX(CN, 20.0);
	liveStemIncrement = tgrowth*liveStemCarbon/(5.0*lifeSpan);
	liveStemCarbon -= liveStemIncrement;
	liveStemNitrogen -= liveStemIncrement/CN;

//Dead stem wood
	deadStemIncrement = liveStemIncrement;
	deadStemCarbon += deadStemIncrement;
	deadStemNitrogen += deadStemIncrement/CN;

//Stem wood residue
	CN = deadStemCarbon/deadStemNitrogen;
	CN = MAX(CN, 20.0);
	residueIncrement = deadStemCarbon*0.01/365.25/48.0;
	stemResidueCarbon += residueIncrement;
	stemResidueNitrogen += residueIncrement/CN;
	deadStemCarbon -= residueIncrement;
	deadStemNitrogen -= residueIncrement/CN;
...
//when N availability is less than 50% then halt stem growth
	if (N_avail_rate_plant > 0.5 && nscRatio > 0.5)
	{
		if(treesParams.useLeafModule == 1)
		{
...
//if leaf allocation is shutting down, then shut down stem allocation as well
	...
	stemCincrement = stemAllocation*rgrowth*1.17;

//check to make sure there is enough NSC for stem growth
	if (stemCincrement > 0.99*stemNSC)
	{
		stemCincrement = 0.99*stemNSC;
	}
//
//Note: This CN calculation needs to be re-evaluated - DSM 2021
//
	CN = 50.0 + (1.0-N_avail_rate_plant)*50.0;
	if (stemCincrement > 0.99*1.17*CN*leafStoredNitrogen)
	{
		stemCincrement = 0.99*1.17*CN*leafStoredNitrogen;
	}
	stemNSC -= stemCincrement;

	N_neg_demand += N_neg_fract*stemCincrement/CN/1.17;
        N_pos_demand += (1.0-N_neg_fract)*stemCincrement/CN/1.17;

	liveStemCarbon += stemCincrement/1.17;
	liveStemNitrogen += stemCincrement/CN/1.17;
	leafStoredNitrogen -= stemCincrement/CN/1.17;
	if (liveStemCarbon < 0.01)
	{
		liveStemCarbon = 0.01;
		liveStemNitrogen = liveStemCarbon/CN;
	}
	if (rgrowth > 0.0)
        {
...
//DSM - July 2015
//
void BiogeochemicalCycles::updateRootCarbonNitrogenPools(
    trees_params &treesParams,//rTREES made by ref 20221020
    std::vector<double> &tempSoil,
    double rgrowth,
    double N_neg_fract,
    double& N_neg_demand,
    double& N_pos_demand,
...
//		temperate trees. New Phytologist, 195, 823-831.
		...
		if (CN < treesParams.fr_minCN)
		{
		  CN = treesParams.fr_minCN;
		}
		rootLifeSpan = refLifeSpan*CN/treesParams.fr_minCN;
...
//compute root mortality
			...
			if (rootCdecrement > 0.1*fineRootBiomassCarbon[j][k])
			{
			  rootCdecrement = 0.1*fineRootBiomassCarbon[j][k];
			}
...
//DSM - July 2020
			...
			rootCincrement = (1.0-leafCfraction-stemAllocation)*rgrowth*tgrowth*rootAllocation * 1.17;
//
//DSM
//Check for error during root C increment
//  -usually occurs when there is hydraulic failure
//  -sensible way to handle this is to halt the simulation
//
//REMOVED 20231113 because this was used to catch hydraulic failure which should have been resolved.
//Left in the message to show there were hydraulic 'failures'
			if (std::isnan(rootCincrement))
			{
			  // if(verbosity>0){
				/*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "BiogeochemicalCycles::updateRootCarbonNitrogenPools():" << "\n";
				/*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << leafCfraction << '\t' << stemAllocation << '\t' << rgrowth << '\t' << tgrowth << '\t' << N_avail_rate_plant << '\t' << rootAllocation << '\t' << CN << "\n";
				// }
				// exit(1);//
			}

			if (fineRootBiomassCarbon[j][k]+rootCincrement/1.17 > maximumRootBiomassCarbon[j][k])
			{
			...
			if (rootCincrement/CN > 0.99 * rootMineralNitrogen[j][k])
			{
				...
				residual *= MIN(1.0,leafStoredNitrogen/leafBiomassNitrogen*(1.0-leafCfraction));
				rootCincrement = 0.99 * CN * rootMineralNitrogen[j][k] + residual;
				rootCincrement *= 1.17;
				leafStoredNitrogen -= residual/CN;
			...
			rootNSC[j][k] -= rootCincrement;
                        fineRootBiomassCarbon[j][k] += rootCincrement/1.17;
                        fineRootBiomassNitrogen[j][k] += rootCincrement/CN/1.17;
			N_neg_demand += N_neg_fract*rootCincrement/CN/1.17;
			N_pos_demand += (1.0-N_neg_fract)*rootCincrement/CN/1.17;
...
//DSM - July 2020
			...
			rootCincrement = (1.0-leafCfraction-stemAllocation)*rgrowth*tgrowth*rootAllocation*1.17;
			if (coarseRootBiomassCarbon[j][k]+rootCincrement/1.17 > maximumRootBiomassCarbon[j][k])
			{
				rootCincrement = (maximumRootBiomassCarbon[j][k]-coarseRootBiomassCarbon[j][k])*1.17;
			}
			if (rootCincrement > 0.99*rootNSC[j][k])
			{
				rootCincrement = 0.99*rootNSC[j][k];
			...
			rootNSC[j][k] -= rootCincrement;
                        coarseRootBiomassCarbon[j][k] += rootCincrement/1.17;
                        coarseRootBiomassNitrogen[j][k] += rootCincrement/CN/1.17;
			//rootMineralNitrogen[j][k] -= rootCincrement/CN/1.17;
			N_neg_demand += N_neg_fract*rootCincrement/CN/1.17;
			N_pos_demand += (1.0-N_neg_fract)*rootCincrement/CN/1.17;
...
//
double BiogeochemicalCycles::root_respiration_rate(
...
//   output units of kgC 30min-1
//
double BiogeochemicalCycles::stem_respiration_rate(
...
//   output units of kgC 30min-1
//
double BiogeochemicalCycles::leaf_respiration_rate(
...
)
{
  ...
  rate = resp_coef_leaf * exp(resp_coefficient*tLeaf) * lai / SLA * 10000.0;
...
//
void BiogeochemicalCycles::computeRootNitrogenUptake(
    std::vector<std::vector<double>> &UP_neg,//[],//[10],
    std::vector<std::vector<double>> &UP_pos,//[],//[10],
						     trees_params &treesParams,//rTREES made by ref 20221020
						     std::vector<double> &thetaSoil,
						     ...
						     double DEM_pos)
{
	...
	double ks, bubbling_pressure, pore_size_index, residual;
...
//
	activeNitrogenDemand = 1.0+ 9.0/(1.0+exp(10.0*plantNstatus-5.0));
	passiveNitrogenDemand = (1.0 - plantNstatus);;
...
//DSM June 2023
		...
		soil_texture_parameters(treesParams, j+3, ks, bubbling_pressure, pore_size_index, residual);
		fieldCapacity = field_capacity(bubbling_pressure, pore_size_index, residual, porosity);
		for (int k = 0; k < nRootOrders; k++)
		{
...
//N solubility declines at lower soil moisture
			if (thetaSoil[j] < fieldCapacity && thetaSoil[j] > 0.000001)
			{
				a *= sqrt(thetaSoil[j]/fieldCapacity+0.000001);
			}
...
//  glucose C cost of (8 x 6 x 12/24)/14 = 1.72 kg C kg-1 N reduced
			...
			if (CostUP > 0.95*leafNSC && CostUP > 0.0)
			{
				UPp *= 0.95*leafNSC/CostUP;
				CostUP = 0.95*leafNSC;
				leafNSC -= CostUP;
			}
			else if (CostUP > 0.0)
...
//convert to g m-2 30min-1
			...
			DEM_neg_A /= pow(plantNstatus+0.0001, 2.0);
...
///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "UPa = " << UPa << "\n";
...
//  glucose C cost of (8 x 6 x 12/24)/14 = 1.72 kg C kg-1 N reduced
			...
			if (CostUP > 0.95*leafNSC && CostUP > 0.0)
			{
				if ((CostUP - 0.95*leafNSC) < 0.99*stemNSC)
				{
					CostUP -= 0.95*leafNSC;
					leafNSC *= 0.05;
					stemNSC -= CostUP;
...
//active ammonium uptake
			...
			DEM_pos_A /= pow(plantNstatus+0.0001, 2.0);
...
//proportion of leaf+root biomass in leaf
	...
	leafToRootCratio = leafBiomassCarbon / (leafBiomassCarbon+2.0*totFineRootC);
...
//This assumes that available NSC determines an optimal amount of available N
	...
	plantNstatus = leafToRootCratio * CN * (leafStoredNitrogen / (getLeafNSC()+0.0000001));
	CN = getFineRootCarbon() / (getFineRootBiomassN()+0.0000001);
	plantNstatus += (1.0-leafToRootCratio) * CN * (getFineRootN() / (getFineRootNSC()+0.0000001));
...
//
//computeLeaching()//PROP
//combine drainage proportion by rhizosphere and N concentration
//drain is in units of m 30 min-1
//
void BiogeochemicalCycles::computeLeaching(
    std::vector<std::vector<double>> LE_neg,//[][10],
    std::vector<std::vector<double>> LE_pos,//[][10],
		     			   trees_params &treesParams,//rTREES made by ref 20221020
		     			   std::vector<double> &thetaSoil,
		     			   std::vector<double> &depthSoil,
					   std::vector<double> &bypassFlow)
{
...
//nitrate leaching
//units are g m-3
			a = 1.0; //for mobile nitrate ion//PROP
			...
			negLE[j] += LE_neg[j][k];
//ammonium leaching
			a = 0.1; //assumes that most ammonium is absorbed in soil matrix//PROP
...
double BiogeochemicalCycles::computeRhizosphereVolume(trees_params &treesParams,//rTREES made by ref 20221020
						      int rootNum)
{
	...
	rhizVolume = 0.0;
//DSM June 2023
	porosity = treesParams.layer_porosity[rootNum+3];
...
//
void BiogeochemicalCycles::updateRhizospherePools(trees_params &treesParams,//rTREES made by ref 20221020
                                                  std::vector<double> &thetaSoil,
						  std::vector<double> &tempSoil,
						  std::vector<double> &depthSoil,
						  std::vector<std::vector<double>> UP_neg,//[],//[10],
...
//DSM June 2023 - porosity and bulk density differ by soil layer
		porosity = treesParams.layer_porosity[j+3];
		bulkDensity = treesParams.layer_BD[j+3];
...
///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << cumDist << "\n";
//PROP Converts from mass per area units to concentration by volume why the difference?
...
//Compute residue input C and C/N to the soil and update residue pools
			if (j == 0) //incorporate aboveground residue and root residue
			{
				...
				{
//add leaf residue
					dC = 0.1/48.0*leafResidueCarbon/(730.0/(fns+0.0001))*rhizVolume/depth;
					dN = dC * leafResidueNitrogen/leafResidueCarbon;
					leafResidueCarbon -= dC;
					leafResidueNitrogen -= dN;
					ADD = dC;
//add stem residue
					dC = 0.1/48.0*stemResidueCarbon/(36500.0/(fns+0.0001))*rhizVolume/depth;
					dN2 = dC * stemResidueNitrogen/stemResidueCarbon;
					stemResidueCarbon -= dC;
					stemResidueNitrogen -= dN2;
					ADD += dC;
				}
				else //annual plants
				{
...
///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "Cb = " << Cb << "; Ch = " << Ch << "; Cl = " << Cl << "\n";
...
//  and this computation is accomplished by using the DAMM (Davidson et al GCB) model
	fds_opt = DAMM_Cflux(treesParams, treesParams.optimal_soil_T, treesParams.theta_opt,
							treesParams.layer_porosity[root_num+3], depth, Cl);
	if (fds_opt > 0.0)
	{
		fds = DAMM_Cflux(treesParams, t_soil, theta, treesParams.layer_porosity[root_num+3], depth, Cl);
		fds = fds/fds_opt;
	}
	else
	{
...
//define the role of degree of saturation on nitrification rate
//Two parts:
...
//
double BiogeochemicalCycles::DAMM_Cflux(trees_params &treesParams,//rTREES made by ref 20221020
                  			double t_soil,
                  			double theta,
                  			double porosity,
                  			double depth,
...
//
double BiogeochemicalCycles::DAMM_reactionVelocity(trees_params &treesParams,//rTREES made by ref 20221020
                                        	   double t_soil,
                                        	   double theta,
                                        	   double porosity,
                                        	   double depth,
...
std::vector<double> BiogeochemicalCycles::getQuantileVals(double alpha, double beta, double min_quant, double max_quant)
{
    ...
    std::gamma_distribution<double> distribution(alpha, theta);
...
double BiogeochemicalCycles::calcRAR(
    double thermCD,
    trees_params& treesParams,
    std::vector<double> Karray,
    std::vector<double> Narray,
...
double BiogeochemicalCycles::calcRER(
    double thermEXP,
    trees_params& treesParams,
    std::vector<double> Karray,
    std::vector<double> Narray,
...
double BiogeochemicalCycles::calcdeltaC(
    double SLA,
    double deltaAreaL
)
{
...
double BiogeochemicalCycles::calcdeltaN(
    double deltaC,
    trees_params &treesParams
)//rTREES made by ref 20221020
{
  double leafCN, SLA_instant, deltaN;
  double N_avail_rate_plant = plantNstatus;
  SLA_instant = calcSLA(N_avail_rate_plant , treesParams.SLA_max, treesParams.SLA_min);
  ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "SLA at this ts of lf is " << SLA_instant << "\n";
...
{
    	//return MIN(NStatus*5.0, 1.0);
    	// this means that if N status is < 0.2, relative growth will be limited
// test new function, informed by Marc Brock's Timmer experiment data from 2016
    	//return(1.0- pow(26.5938, -1.0*NStatus));
    	//return MIN( (1.0- pow(26.5938, -1.0*NStatus))*2.0, 1.0 );
    	// try this -- less sensitivity
    //return(1.0);
...
void BiogeochemicalCycles::updateLeaf(
    int k,
    ...
    std::vector<double> Narray,
    std::vector<double> rarray,
    // double* Karray, double* Narray, double* rarray,
    int idx, double thetaRoot
                                        )
{
      ...
      ((SingleLeafArea[k] < SingleLeafAreaPotential[k] ) ) )
  {
        ...
        (SingleLeafArea[k] <= K ) )
    {
      ...
      N_RAR_coef = calcNRAR(plantNstatus);
      deltaApot = RAR * SingleLeafAreaPotential[k] * delta_thermTime * water_RAR_coef * N_RAR_coef;

      if ((SingleLeafAreaPotential[k] + deltaApot) > K)// if the change will bring total greater than max
      {
        deltaApot =  K - SingleLeafAreaPotential[k];
      }
      SingleLeafAreaPotential[k]  += deltaApot;
      Rcd = calcRespCD(deltaApot, 1.0*pow(10.0,-50.0)); // kgC
      leafNSC -= Rcd/cm2_to_ha(treesParams.pot_size);
      ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "cost of cd, kgC, = " << Rcd << "\n";
    }
    ...
    if (SingleLeafThermTm[k] > ((1.0-treesParams.proportion_CD)* duration_Leaf))
    {
      ...
      waterStat = thetaRoot/treesParams.layer_porosity[3];
      if (waterStat > 1.0) waterStat = 1.0;
      ...
      SLA_instant = calcSLA(plantNstatus, treesParams.SLA_max, treesParams.SLA_min);
      // returns in kgC/ha
      double projGNDarea = calc_projGndArea(treesParams, Karray);
      //C_needed = calcdeltaC(SLA_instant, deltaAreaL)/cm2_to_ha(treesParams.pot_size);
      C_needed = calcdeltaC(SLA_instant, deltaAreaL)/cm2_to_ha(projGNDarea);
      NSCStatus = leafNSC;
      ...
      //TEMP TURNED BACK ON MCH 20211122
      N_RAR_coef = calcNRAR(plantNstatus);
      ...
      //update area, cm2, now computed in updateLeafCarbonNitrogenPools
      //            		SingleLeafArea[k] = SingleLeafArea[k] + deltaAreaL;
      ...
      ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "deltaN at this ts is " << deltaN << "\n";
...
double BiogeochemicalCycles::calcPlantSLA(
    int Lf_idx,
    trees_params &treesParams)//rTREES made by ref 20221020
{
  double sum_area;
...
double BiogeochemicalCycles::calcLAI(
  int Lf_idx,
  trees_params &treesParams,//rTREES made by ref 20221020
  std::vector<double> Karray
  // double* Karray
...
{
  double sum_ProjArea;
  double max_ProjArea, len, len_proj;//, lai;//rTREES unused
  double avg_K;
  double sum_K = 0.0;
  ...
  avg_K = sum_K/500.0;
  ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << avg_K << "\n";
...
double BiogeochemicalCycles::calc_projGndArea(
    trees_params &treesParams,//rTREES made by ref 20221020
    std::vector<double> Karray
    // double* Karray
)
...
double BiogeochemicalCycles::calc_projArea(
    int Lf_idx,
    trees_params &treesParams)//rTREES made by ref 20221020
{
	double sum_ProjArea;
...
double BiogeochemicalCycles::getSumLeafBiomassCarbon(
    int Lf_idx,
    double ProjGrndArea_instant
)
{
...
double BiogeochemicalCycles::getSumLeafBiomassNitrogen(
    int Lf_idx,
    double ProjGrndArea_instant
)
{
