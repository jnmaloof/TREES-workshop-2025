...
int read_time(std::vector<double> in_time, Time& tm)
{
	int yd;
        double hm;
        int y, j, h, m;
        ...
        y = in_time[0]; //yd/1000; //throwing out the last 3 digits
        j = in_time[1]; //yd%1000; //the rest is the day
        // hm = hm*100; //last 2 digits = mins
        h = in_time[2];//(int)hm/100;
        m = in_time[3]; //(int)hm%100;
...
// std::istream& read_time_stamp(std::istream& in, Time& tm)
// {
// 	int y, j, h, m;
// 	char tempch;
...
// std::istream& read_time_tdat(std::istream& in, Time& tm, int year)
// {
//         int j, h, m;
// 	double temp;
//
...
int Time_Array::allocate(int max)
{
	...
	if (max > 0)
	{
		series = new Time[max];//rTREES watch
		if (series != NULL)
		{
			flag = steps = max;
		}
...
// std::istream& read_step(std::istream& in, Time_Array& ta, int step)
// {
// 	if (step < ta.steps)
// 	{
...
// std::ostream& print_step(std::ostream& out, const Time_Array& ta, int step)
// {
// 	if (step < ta.steps)
// 	{
// 		out << ta.series[step];
