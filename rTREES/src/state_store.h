...
//inserting values in a State_Store
	...
	//for var name vname
		int set_val_for(double val, const std::string& vname);
	//update a value for a state variable; returns 0 if fails, 1 OK
	//at index ind
		int update_val_at(double delta_val, int ind);
	//for var name vname
		int update_val_for(double delta_val, const std::string& vname);
