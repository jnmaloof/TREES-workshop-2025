...
//inserting values in a Data_Store
	...
	//for var name vname
		int put_val_for(double val, const std::string &vname, int st);
        //copy the whole col of variable vname from orig 
    int insert_col(std::string &vname, const Data_Store& orig);
  //check which index a string is stored at
   int get_index_for(const std::string &vname);
   //check which name is at that index
   std::string get_name_for(const int ind);
...
//data verification & access
	...
	//by var name
		double get_val_for(const std::string &vname, int st) const;
...
//returns the number of variables available
	...
	private:
		...
		std::vector<std::vector<double> >  values;           //data values for variable @ each time step
};
