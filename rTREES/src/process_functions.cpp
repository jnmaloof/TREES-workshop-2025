                ...
                double longi)
{
  ...
  corr += (-104.7*sin(temp) +596.2*sin(2.0*temp) +4.3*sin(3.0*temp) -12.7*sin(4.0*temp)
             -429.3*cos(temp) -2.0*cos(2.0*temp) +19.3*cos(3.0*temp))/3600.0;
                       ...
                       )
{
  ...
  double daily_pd_lwp, nsc, unstressedLeafLifeSpan, stressedLeafLifeSpan;

  double lai_pot;

  lai_pot = state.get_val_at(CURRENTTARGETLAI);

  unstressedLeafLifeSpan = treesParams.leafLifeSpan;
  stressedLeafLifeSpan = treesParams.leafLifeSpan * RL;
  if (stressedLeafLifeSpan < 1.0)
  {
    stressedLeafLifeSpan = 1.0;
  }
  if (ts == 1)
  {
    simOut.sum_vpd = 0.0;
  ...
  {
    lai_pot = (1.0-1.0/stressedLeafLifeSpan)*state.get_val_at(CURRENTTARGETLAI) +
      state.get_val_at(FORECASTTARGETLAI)/stressedLeafLifeSpan;
    if (RL < 1.0)
    {

        lai_pot = state.get_val_at(FORECASTTARGETLAI);
  ...
  if (ts < 48)
  {
    treesParams.lai = lai_pot * (1.0 - 1.0/unstressedLeafLifeSpan);
  }
  if (treesParams.lai < 0.01)
  {
    treesParams.lai = 0.01;
  }
  //added to handled leaf water potential//This will only run at time 0300
  if ((hour+(min/MINS)) == 3.0 && treesParams.useHydraulics == true)
  {
  ...
  delta_nsc = 0.0;
  if (hour+(double)min/(double)MINS == 23.5 && ts >= 48)
  {
    ...
    if (lai_scalar >= 0.0)
    {
      //lai is scaled over the annual change based on leaf lifespan
      lai_scalar = 1.0-1.0/stressedLeafLifeSpan+lai_scalar/stressedLeafLifeSpan;
      //treesParams.lai contains actual LAI from theee previous time step
      old_lai = treesParams.lai;
      //first determine the potential change in LAI without water, carbon, and nitrogen stress
      delta_lai = lai_pot * lai_scalar - old_lai;
    }
    //call function to compute actual leaf carbon, nitrogen, and update resource pools
    bgc.updateLeafCarbonNitrogenPools(treesParams, delta_lai, RL,
                                      N_neg_fract, N_neg_demand, N_pos_demand);
    //new LAI is based on actual realized leaf carbon and SLA
    // Rcpp::Rcout<<"Testing_LAI issue "<<
    //   "bgc.getLeafBiomassCarbon() "<<bgc.getLeafBiomassCarbon()<<"\n"<<
    //     "treesParams.SLA "<<treesParams.SLA<<"\n"<<
    //       " treesParams.lai "<< treesParams.lai<<"\n"<<
    ...
    //                     "GSI_sum "<<GSI_sum<<"\n"<<
    //                       "dayindex "<<dayindex<<"\n"<<
    //                         "treesParams.SLA_instant "<<treesParams.SLA_instant<<"\n"<<
    //                     "RL "<<RL<<"\n"<<
    //                 "N_neg_fract "<<N_neg_fract<<"\n"
    //   <<"\n";
                ...
                )
{
  ...
  //
  //Modified - DSM May 2023
  //LWP should not be allowed GSI to drop below 0.5
  //
  lwpmin = -9.0;
  lwpmax = -5.3;
  if (gsi_lwp <= lwpmin)
  ...
  {
    lwp_indicator = 0.5 + 0.5*(gsi_lwp-lwpmin)/(lwpmax-lwpmin);
  }
                        ...
                        double pressure)       //atmospheric pressure, kPa
{
  ...
  /*
   if(i >= n_times)
   std::cout << "WARNING! " << n_times << " iterations exceeded.\n";
   */
  return (zeta);
}
                        ...
                        double theta)
{
  ...
  S = (theta - residual) / (porosity - residual);//van Ganuchten 1980 [2]
  if (S < 0.001)
  {
    S = 0.001;
  }
  ...
  //Use van Ganuchten model of soil water potential//should be 0.0001 for proper unit conversion
  // psi_soil = -0.0001019977334*bubbling_pressure * pow(pow(S,-1.0/m)-1.0, 1.0/n);//van Ganuchten 1980 [24?29b?]
  psi_soil = -0.00009806652*bubbling_pressure * pow(pow(S,-1.0/m)-1.0, 1.0/n);//van Ganuchten 1980 [24?29b?]
  if (psi_soil < -1000.0)
  {
    psi_soil = -1000.0;
  }
...
//Only used if hydraulic model is disabled
                                ...
                                double field_capacity)
{
  ...
  psi_soil = 0.00009806652*bubbling_pressure * pow(pow(S,-1.0/m)-1.0, 1.0/n);//Prop//Why is this one different?
...
void soil_texture_parameters(	trees_params& treesParams,
                              ...
                              double& residual)
{
  double por = treesParams.layer_porosity[j];
  double pClay = 100.0*treesParams.layer_clay_fraction[j];
  double pSand = 100.0*treesParams.layer_sand_fraction[j];

  soil_texture_parameters(por, pClay, pSand, ks, bubbling_pressure, pore_size_index, residual);
  residual = treesParams.layer_residual_water_content[j];
}
                      ...
                      double mineral_fraction)
{
  ...
  psi2 = -2243.95;//vaporization critical point (22MPa)
...
//
///This may be causing numerical issues!
//MCH I added in the isnan and other checks to the updated valued except the last one setting to max at porosity. add throw in here and currant number scaling in unsat_zone_drainage
                    ...
                    )
{
  ...
  double porosity;
  double bubbling_pressure, residual, pore_size_index,ks;
  ...
  while (kiter > 0)
  {
    ...
    for (int i = 0; i < rmodules; i++)
    {
      //DSM June 2023 - soil parameters by layer
      ...
      if (updatedTheta > porosity)
      {
        ...
        updatedTheta = porosity;//why / then *(rootDepth[i]*mineral_fraction) go? ???
      }
      if (i < rmodules-1)
      {
        ///MCH check if time reduction is not enough
        //calculate the currant number
        //have it reduce time step based on courant number
        dt_x=0.5*(kinter/ks)*updatedTheta*(MIN(rootDepth[i],rootDepth[i+1]));



        // Rcpp::Rcout<<"..................\n";
        // Rcpp::Rcout<<"> kinter "<<kinter<<"\n";
        // Rcpp::Rcout<<"> worst Co "<<(ks/kinter)/updatedTheta*30/(MIN(rootDepth[i],rootDepth[i+1]))<<"\n";
        // Rcpp::Rcout<<"> worse case ts "<<dt_x<<"\n";

        // Rcpp::Rcout<<"#:rd1:\n"<<rootDepth[i]<<",\n#:rd2:\n"<< rootDepth[i+1]<<",\n#:pidx:\n"<<pore_size_index<<",\n#:ko:\n"<<ks/kinter<<",\n#:ut:\n"<<updatedTheta<<",\n#:ts2:\n"<<thetaSoil[i+1]<<",\n#:fc:\n"<<field_capacity<<",\n#:rd:\n"<<residual<<",\n#:cf:\n"<<capFringe<<",\n#:p:\n"<<porosity<<",\n#:mf:\n"<<mineral_fraction<<"\n";

        drain += unsat_zone_drainage(
          rootDepth[i],
          rootDepth[i+1],
          pore_size_index,
          ks/kinter,// k_theta???
      ...
      bypassFlow[i] += bypass;
      // if(std::isnan(thetaSoil[i])){
      //   // thetaSoil[i]=0.0;
      // }
      // if(std::isnan(bypassFlow[i])){
      //   // bypassFlow[i]=0.0;
      // }
      // if(thetaSoil[i]<residual){
      //   // thetaSoil[i]=residual;
      // }
    }
    drain = delta_Sr = 0.0;
  }
  ...
  // if(std::isnan(thetaSoil[rmodules-1])){
  //   // thetaSoil[rmodules-1]=0.0;
  // }
}
                           ...
                           double mineral_fraction)
{
  ...
  if (theta1 > 0.9*porosity)
  {
    ...
    ko = ko/30.0;
    // Rcpp::Rcout<<1<<" second ts"<<"\n";
  }
  else if (theta1 > 0.67*porosity)
  {
    iter=6;
    ko = ko/6.0;
    // Rcpp::Rcout<<5<<" second ts"<<"\n";
  ...
  for (int i = 0; i < iter; ++i)
  {
    ...
    //ku = ko * pow(S, n) * pow(1.0 - pow(1.0 - pow(S, 1.0/m), m), 2.0);
    ku = ko * pow(S, 0.5) * pow(1.0 - pow(1.0 - pow(S, 1.0/m), m), 2.0);// %%%K
  ...
  //   Rcpp::Rcout<<"\n\n~~~~~~~~~~~~~~~~~~~~~~~\n";
  //   Rcpp::Rcout<<"~~~~~~~~~~~~~~~~~~~~~~~\n";
  //   Rcpp::Rcout<<"> ttl drain "<<totalDrain<<"\n";
  //   Rcpp::Rcout<< dZ1<<",\n";
  //   Rcpp::Rcout<< dZ2<<",\n";
  //   Rcpp::Rcout<< pore_size_index<<",\n";
  //   Rcpp::Rcout<< ko<<",\n";
  //   Rcpp::Rcout<< theta1<<",\n";
  //   Rcpp::Rcout<< theta2<<",\n";
  //   Rcpp::Rcout<< field_capacity<<",\n";
  //   Rcpp::Rcout<< residual<<",\n";
  //   Rcpp::Rcout<< air_entry_pressure<<",\n";
  //   Rcpp::Rcout<< porosity<<",\n";
  //   Rcpp::Rcout<< mineral_fraction<<",\n";
  //   Rcpp::Rcout<<"\n\n~~~~~~~~~~~~~~~~~~~~~~~\n";
  //   Rcpp::Rcout<<"~~~~~~~~~~~~~~~~~~~~~~~\n";
  // }
    ...
    double gv)         //vapor cond. (both cond. mol m-2 s-1)
{
  ...
  /*
   ...
   std::cout << lambda << '\t' << l_mol << '\t' << gamma_star  << '\t'
             << ess << '\t' << Rec << '\t' << Dec << '\t' << Ecanopy << '\t';
   */
