...
void DoHydraulicModel(bool silent,
			...
			std::vector<double> &ecritOut, std::vector<double> &pc, std::vector<double> &klpred,
			std::vector<double> &ppredOut, std::vector<double> &epredOut, std::vector<double> &nodeFail,
			std::vector<char> &nodeTyp,std::vector<std::vector<std::vector<double> > > &psi,std::vector<std::vector<double> > &rflux,
			double soilpsiavg[], double nts[],
			...
			double vu[][ULAT], double vl[][ULAT],
                    	double cf[][ULAT],
                                std::vector<std::vector<double>> &pe, //RTEES watch
			double ws_[][ULAT],
                    	...
                    	int &tnode, double fac, double &modelledKL)
{
  ...
  if (ev < 0.0001*treesParams.e_at_saturated_kl)
  {
    ev = 0.0001*treesParams.e_at_saturated_kl;
  }
...
void HydraulicModel::readin(bool silent, bool callRewet,
				double soilpsi[ULAT], double soilpsiavg[1], double ks[][ULAT],
            			double pressure[NKINC], double plc[NKINC],
				std::vector<std::vector<double> > &l_raw,
            			...
            			double p[MD][ULAT],
				std::vector<double> &md_raw,
				std::vector<double> &e_raw,
				std::vector<double> &la_raw,
				std::vector<double> &arAl,
				double cc[MD][2],
	    			...
	    			trees_params& treesParams)
{
  ...
  else
  {
        ...
        if ( i != (ShootModules+1))  // skip over "hidden" module
        {
          if (i <= ShootModules)
          {
            if (treesParams.allowLeafRefilling == true)
            {
              ...
              //It reduces the allowable leaf refilling as LAI increases
              //  lratio = 0.0 --> full potential to refil leaf
              //  lratio = 1.0 --> no potential to refill leaf
              //
              //For species other Maize, lratio = 0.0 (for now)
              //DSM - 12/24/2021
              //
              double lratio, delta_lai, nleaves;
              lratio = 0.0;
              if (treesParams.useLeafModule == false && treesParams.usePhenology == false)
              {
                //
                //linear change in refilling should be turned off for now -- 7/8/2022 DSM
                //								lratio = latot/max(0.1,treesParams.laiFullCanopyHeight);
                //
                if (lratio > 1.0)
                {
                  lratio = 1.0;
                }
                psimin[i][1] = lratio*psimin[i][1]+(1.0-lratio)*soilpsiavg[t];
                /*
                 delta_lai = latot - treesParams.previous_lai;
                 cout << psimin[i][1] << '\t';
                 psimin[i][1] = (treesParams.previous_lai*psimin[i][1] + delta_lai*soilpsiavg[t])/latot;
                 cout << treesParams.previous_lai << '\t' << delta_lai << '\t' << soilpsiavg[t] << '\t' << psimin[i][1] << endl;
                 //psimin[i][1] = soilpsiavg[t];
                 treesParams.previous_lai = latot;

                 lratio = 1.0 - exp(-0.5*(latot-0.5));
                 if (lratio < 0.0)
                 {
                 lratio = 0.0;
                 }
                 lratio = 1.0-exp(-pow(latot/max(0.1,0.63*treesParams.laiFullCanopyHeight), 3.5));
                 //								psimin[i][1] = (lratio*treesParams.previous_lai*psimin[i][1] + delta_lai * soilpsiavg[t]) / (lratio*treesParams.previous_lai + delta_lai);;
                 */
              }
              else
              {
    ...
    if (!silent)//removing this had some very unintentional consequences. This needs to be cleaned up but the xylemScalar logic below needs to be traced
    {
      if (xylemScalar < 1.0)
      {
        /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout <<"\n"  << "-> Predawn: updating Weibulls with xylem loss = " <<100.0*(1.0-xylemScalar) << "%" <<"\n" ;
      ...
      {
        /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout <<"\n"  << "-> Predawn: updating Weibulls with xylem loss = " <<100.0*(xylemScalar-1.0) << "%" <<"\n" ;

      }
      else
      {
        ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout <<"\n"  << "-> Predawn: updating Weibulls with soil Psi change = " <<
        //		 delta <<"\n" ;
      }
    }
    pdecrement = 0.01;
...
void HydraulicModel::solveWater(
    bool silent, bool callRewet, double Ypd[],
...
)
{
  ...
  std::vector<double> md_raw(MD), e_raw(MD), la_raw(MD), arAl(MD);
  std::vector<std::vector<double> > l_raw(5,std::vector<double>(MD));
  ...
  { //this loop increments Q
    ...
    { //this loop finds steady-state at each Q
      ...
      { //this loop finds new potentials and contents at each time step
        ...
        { //get upper and lower k's for axial shoot elements
               ...
               !verifyExpCondition(result2) )
          {
            ...
            nodefailure = i;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 2 " <<"\n" ;
            //			    			nodetype = "axial shoot";
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 4c\n";
            goto innerWhile1;
          }
          ...
          if ( pgamma(aa, nn, fac, x, gamma, gammp) )
          {
            ...
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 3" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 4b\n";
            goto innerWhile1;
          }
          ...
          if ( pgamma(aa, nn, fac, x, gamma, gammp) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //			    			nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 4" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 4a\n";
            goto innerWhile1;
          }
        ...
        { //get upper and lower k's for axial rhoot elements
              ...
              !verifyPowCondition(base2,exponent) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //			    			nodetype = "axial root";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 5" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 3d\n";
            goto innerWhile1;
          }
               ...
               !verifyExpCondition(result2) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //			    			nodetype = "axial root";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 6" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 3c\n";
            goto innerWhile1;
          }
          ...
          if ( pgamma(aa, nn, fac, x, gamma, gammp) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //			    			nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 7" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 3b\n";
            goto innerWhile1;
          }
          ...
          if ( pgamma(aa, nn, fac, x, gamma, gammp) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //			    			nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 8" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 3a\n";
            goto innerWhile1;
          }
        ...
        { //get upper and lower k's for lateral shoot elements
              ...
              !verifyPowCondition(base2,exponent) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //			    			nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 9" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 2d\n";
            goto innerWhile1;
          }
               ...
               !verifyExpCondition(result2) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //			    			nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 10" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 2c\n";
            goto innerWhile1;
          }
          ...
          if ( pgamma(aa, nn, fac, x, gamma, gammp) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //			    			nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 11" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 2b\n";
            goto innerWhile1;
          }
          ...
          if ( pgamma(aa, nn, fac, x, gamma, gammp) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //			    			nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 12" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 2a\n";
            goto innerWhile1;
          }
        ...
        { //get upper and lower k's for lateral root elements
          // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd 1139 \n";
              ...
              !verifyPowCondition(base2,exponent) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //						nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 13" <<"\n" ;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 1d\n";
            goto innerWhile1;
          }
               ...
               !verifyExpCondition(result2) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //						nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 14" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 1c\n";
            goto innerWhile1;
          }
          ...
          //weibull function for upper half of element i
          ku[i][1] = ks[i][1] * exp( -result2 );
          // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd 1179 \n";
          ...
          if ( pgamma(aa, nn, fac, x, gamma, gammp) )
          {
            ...
            //HydraulicModelFailCond = 1;
            //						nodetype = "lateral shoot";
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 15" <<"\n" ;
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 1b\n";
            goto innerWhile1;
          }
          ...
          if ( pgamma(aa, nn, fac, x, gamma, gammp) )
          {
            ...
            //HydraulicModelFailCond = 1;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "failure 16" <<"\n" ;
            nodefailure = i;
            //						nodetype = "lateral shoot";
            error = true;
            ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "from 1a\n";
            goto innerWhile1;
          }
          // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd 1217 \n";
        ...
        for (i = ShootModules + 2; i <= (totmodules + 1); i++ )
        {
          ...
          for (j = 2; j <= SoilRhizElements + 1; j++ )
          {
            ...
            else
            {
              ...
              // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd 1251 \n";
      ...
      innerWhile1: /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout <<"";
        // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd innerWhile1 \n";
    ...
    innerWhile2: /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout <<"";
      // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd innerWhile2 \n";
      ...
      // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd 1624 \n";

      pleafave = Sum / ShootModules;
      ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "pleafave " << pleafave <<"\n" ;
      ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "ptarg " << ptarg <<"\n" ;
      //if (pleafave == ptarg) {
      //WHY WAS THIS SET TO e == evap[i]?
      // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd 1633 \n";

      if (pleafave > ptarg)
      {
        // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd 1641 \n";
        //	    	if(e == evap[ii])
        //{
        ...
        {    //if no e measurements][get pressures from p data
          // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd 1649 \n";
          ...
          for ( i = ShootModules; i <= totmodules+1; i++)
          {
            ...
            if ( i != ShootModules+1)
            {
              // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd 1658 \n";
  ...
  outerWhile: /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout <<"";
    error = false;
  ii--;
  check = 0;
  check = 0;
  ...
  {     //no predictions or pressures
    ...
    check = 1;
    ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "no pred" <<"\n" ;
    epred = 0.0;
    ppred = 0.0;
    for ( i = ShootModules; i <= totmodules + 1; i++)
    {
  ...
  else
  {
    ...
    if (ptarg != 0.0)
    {
      ...
      if (ptarg < pcrit)
      {
        ...
        /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "fail " << "\n";
        /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "ptarg " << ptarg << "\n";///MCH_ watch this for


        ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "pcrit " << pcrit <<"\n" ;
        HydraulicModelFailCond = 1; //notifies Trees of failure
        nodeFail[t] = nodefailure;
        //		    		nodeTyp[t] = nodetype;
        check = 1;
        // std::cin.get();//Turn this into an error function
      }
    }
    if (e != 0.0)
    {
      ...
      {
        /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "fail " << "\n";
        /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "e " << e << "\n";
        /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "ecrit " << ecrit << "\n";
...
//rTREES removed. I don't think this was used and all output is R format now anyways.->>
...
// 					ecrit_k_psi.rhizK[i] << " " << ecrit_k_psi.rhizFlux[i] <<"\n" ;
//         }
//         outData.close();
//
...
// 	{
//             	outData << saturatedKs[i][1] <<" "<< saturatedKs[i][2] <<" "<< saturatedKs[i][3] <<"\n" ;
//         }
//         outData.close();
// }
//||end rTREES removed.
...
)
{
  ...
  // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd _  2079 \n";

}

//Setup is called at the beginning of simulation ~~only~~ and every 10 days (anuals) or yearly( perenials)
void HydraulicModel::setup(
  bool reset,
...
)//rTREES made by ref 20221020
{
  ...
  double GMP = treesParams.GMP;///MCH_
  ...
  rr = 0.0001;//root radius in m ///MCH_ why this val?
  rzw = rhizosphere_width;//mm

  rzw = rzw * 0.001;//rhizosphere width, converted to m
  rd = pow( ( 1.0 / rzw ), 2.0 ) / M_PI;//root length density

  SoilRhizElements = soilshells;
  dt = static_cast<double>(time_steps);// (double) time_steps;
  gmd = GMP;///MCH_ these only appear to be used in initialize and may be removeable
  gsd = GSD;///MCH_
  bkd = BD;///MCH_
  fs = silt_fraction;///MCH_
  fc = clay_fraction;
  ...
  ksolve(  treesParams, r, pe, b, b1, n, n1, ks, ws_, rssoil, cf, vu, vl, saturatedKs, cpp,
           ...
           soilvol,
           // initial_conductivity_root, decrement_root, initial_conductivity_shoot,         decrement_shoot,
...
void HydraulicModel::ksolve(
  ...
  double & raxial,
  double raxlat,
  const int ShootModules,
  const int totmodules,
  double ar[],
  double axr[],
  double drax[],
  double latr[],
...
{
  bool unsolved_sat = true;
  double failed_solver;//this should never be necessary, but left for now.
  double rlateralmax=10000.0;//how high should this be?
  double rlateralmin=0;
  double slateralmax=10000.0;
  double slateralmin=0;
  // double frlateral;
  int i=0, j=0, k=0, t=1;
  // double increment=0.0, peter=0.0, pRoot=0.0, pLeaf=0.0;
  double Sum=0.0, saxial=0.0;
  ...
  // frlateral = initial_conductivity_root/MIN(1.0, latot);
  ...
  do
  {
    ...
    rlateral=(rlateralmax+rlateralmin)/2.0; //#test at midpoint
    //if midpoint is two high new max is midpoint
    //if midpoint is too low new min is midpoint
    //min can be 0 and max can be insane and still solve in a few steps

///original
    // rlateral -= increment;  // decrement k's until pressures match actual
    ...
    solvek( treesParams, r, pe, b, b1, n, n1, ks, ws_, rssoil, cf, vu, vl, saturatedKs,
...
// }
    ...
    if(
      fabs(p[ShootModules+1][0]-(soilpsiavg[t]-(rfract*(soilpsiavg[t] - ptarg))))<0.00001
        ){
      unsolved_sat=false;
    }else if(
        p[ShootModules+1][0] >(soilpsiavg[t]-(rfract*(soilpsiavg[t] - ptarg)))
    ){
      rlateralmax=rlateral;
    }else if(
        p[ShootModules+1][0] < (soilpsiavg[t]-(rfract*(soilpsiavg[t] - ptarg)))
    ){
      rlateralmin=rlateral;
    }else{
      failed_solver=101.0;
    }
...
&&  failed_solver<100.0
  ...
  if(failed_solver>=100.0){
    treesParams.softerror=treesParams.maxsofterror+999;
    Rcpp::Rcout<<"ERROR! root pressure solver failed!";
  ...
  // Rcpp::Rcout<<"Solver1 took "<<failed_solver<<" steps\n";
  ...
  do
  {
    ...
    slateral=(slateralmax+slateralmin)/2.0;
    ...
    solvek( treesParams, r, pe, b, b1, n, n1, ks, ws_, rssoil, cf, vu, vl, saturatedKs, cpp,
    ...
    if(
      fabs(pleafave - ptarg)<0.00001
    ){
      unsolved_sat=false;
    }else if(
        pleafave > ptarg
    ){
      slateralmax=slateral;
    }else if(
        pleafave < ptarg
    ){
      slateralmin=slateral;
    }else{
      failed_solver=101.0;
    }

  } while ( unsolved_sat && failed_solver<100.0
              );    // loop until pressures match

  if(failed_solver>=100.0){
    treesParams.softerror=treesParams.maxsofterror+999;
    Rcpp::Rcout<<"ERROR! shoot pressure solver failed!";
  }

  // Rcpp::Rcout<<"Solver2 took "<<failed_solver<<" steps\n";
...
)
{
  ...
  // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"hyd solvek \n";
  ...
  initialize( treesParams, r,  pe,  b,  b1, n,  n1,  ks,  ws_, rssoil,  cf,  vu,  vl,
  ...
  do
  {
    ...
    do
    {
      ...
      { //get upper and lower k's for axial shoot elements
        ...
        if( !verifyPowCondition(base1,exponent) ||
            !verifyPowCondition(base2,exponent) )
        {
          ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "ERROR pow AT 1a\n";
          exitError = true;
          goto exitInner;
        }
        ...
        {
          ///*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "ERROR pow AT 1b\n";
          exitError = true;
          goto exitInner;
        }
    ...
    exitInner: /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"";
  ...
  exitOuter: /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout<<"";
    //calculate pleafave
...
void HydraulicModel::initialize(
    trees_params& treesParams,
    double r[MD][ULAT],
    std::vector<std::vector<double> > &pe,
    double b[MD][ULAT],
...
)
{
  ...
  for (i = ShootModules + 2; i <= totmodules + 1; i++ )
  {
    //DSM June 2023 - GMD, GSD, bulk density, clay fraction, silt fraction  by soil layer
    gmd = treesParams.layer_GMP[i];
    gsd = treesParams.layer_GSD[i];
    bkd = treesParams.layer_BD[i];
    fc = treesParams.layer_clay_fraction[i];
    fs = 1.0 - fc - treesParams.layer_sand_fraction[i];
    for (j = 2; j <= SoilRhizElements + 1; j++ )
    {
      pe[i][j] = -0.5 * pow( gmd , (double) -0.5 ); //eqn 5.10 in campbell //PROP
  ...
  for (i = ShootModules + 2; i <= totmodules + 1; i++ )
  {
    ...
    for (j = 2; j <= SoilRhizElements + 1; j++ )
    {
      //conductance factor: cf x soil conductivity = soil element conductance in v/t MPa /MCH radial segment 20231127
                      		...
                      		double &gamma, double &gammp)
{
  ...
  if ( x < ( aa + 1.0 ) )
  {
    ...
    if( error )
    {
    //  /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "ERROR in pgamma\n";
      error = true;
      goto stop;
    }
    gammp = gamser;
...
bool HydraulicModel::gser(double aa, double nn, double &gamser, double &x, double &gln)
{
  ...
  else
  {
    ...
    do
    {
      ...
      {
      //  /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "FOUND ERR\n";
        return true;
      }
      Sum += del;
      k++;
