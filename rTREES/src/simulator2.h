...
// std::vector<std::vector<double>>* get3DArray(int x, int y, int z);
// std::vector<std::vector<double>> get2DArray(int x, int y);
...
struct trees_params {
    ...
    double layer_GSD[MD];
    double layer_BD[MD];
    double layer_porosity[MD];
    double layer_sand_fraction[MD];
    double layer_clay_fraction[MD];
    double layer_residual_water_content[MD];
    double layer_initial_water_content[MD];
    // end June 2023 additions
...
public:
    ...
    double C4photosynthesis(trees_params& treesParams, // rTREES made by ref 20221020
        double Ca,
        double Iin,
        double phiJ,
        double Tl,
        ...
        double& Ci,
        double& Cm);
    void coupledA4_gc(trees_params& treesParams, // rTREES made by ref 20221020
        double Ca,
        double Iin,
        double phiJ,
        double Tl,
        ...
        double& Ci,
        double& Cm);
    double C3photosynthesis(trees_params& treesParams, // rTREES made by ref 20221020
        double Ca,
        double Pa,
        double Iin,
        double phiJ,
        ...
        double& NG,
        double& NS);
    void coupledA3_gc(trees_params& treesParams, // rTREES made by ref 20221020
        double Ca,
        double Pa,
        double Iin,
        double phiJ,
    ...
    double computeRootArea(trees_params& treesParams); // rTREES made by ref 20221020
    double computeRootArea(trees_params& treesParams, // rTREES made by ref 20221020
        int j);
    double computeRootArea(trees_params& treesParams, // rTREES made by ref 20221020
        double SRA,
    ...
    double computeFineRootArea(trees_params& treesParams); // rTREES made by ref 20221020
    double computeFineRootArea(trees_params& treesParams, // rTREES made by ref 20221020
        int j);
    double computeFineRootArea(trees_params& treesParams, // rTREES made by ref 20221020
        double SRA,
    ...
    void computeMaximumRootBiomassCarbon(trees_params& treesParams); // rTREES made by ref 20221020

    // method to compute canopy cover as a proportion of the maximum ground area based on lateral root extension
    double computeCanopyCover(trees_params& treesParams); // rTREES made by ref 20221020
    ...
    void computeNSCfluxes(trees_params& treesParams, // rTREES made by ref 20221020
        double kratio,
        std::vector<double>& kratio_vector);

    // method to compute N fluxes
    void computeNfluxes(trees_params& treesParams, // rTREES made by ref 20221020
        double kratio,
        std::vector<double>& kratio_vector);

    // method to compute root exudates
    void computeRootExudates(trees_params& treesParams, // rTREES made by ref 20221020
    ...
    void updateStemCarbonNitrogenPools(trees_params& treesParams, // rTREES made by ref 20221020
        double N_avail_rate_plant,
        double kratio,
        double nscRatio,
        double rgrowth,
    ...
    void updateRootCarbonNitrogenPools(trees_params& treesParams, // rTREES made by ref 20221020
    ...
    double computeRhizosphereVolume(trees_params& treesParams, // rTREES made by ref 20221020
        int rootNum);
    double computeRhizosphereVolume(trees_params& treesParams, // rTREES made by ref 20221020
        int rootNum,
        int rootOrder);
    ...
    void updateRhizospherePools(trees_params& treesParams, // rTREES made by ref 20221020
        std::vector<double>& thetaSoil,
        std::vector<double>& tempSoil,
        std::vector<double>& depthSoil,
        std::vector<std::vector<double>> UP_neg, //[][10],
    ...
    double computeMineralizationImmobilization(double& MIN,
        ...
        double AtoVbulk,
        double depth,
        trees_params& treesParams); // rTREES made by ref 20221020 plus added the treesParams
    /*
                    double computeAmmonification();
                    double computeNitrification();
    */
    double nitrification_water_function(double theta_opt,
        double theta_soil);
    ...
    double DAMM_Cflux(trees_params& treesParams, // rTREES made by ref 20221020
        double t_soil,
        double theta,
        double porosity,
        double depth,
...
public:
    ...
    std::vector<double> heterotrophicRespiration;
...
private:
    ...
    std::vector<std::vector<double>> rootResidueCarbon;
    std::vector<std::vector<double>> rootResidueNitrogen;
    // Flux variables for root residue inputs
    std::vector<std::vector<double>> dCrootResidue;
    std::vector<std::vector<double>> dNrootResidue;
    // State variables for stabilized soil carbon
    std::vector<double> humusCarbon;
    std::vector<double> humusNitrogen;
    // State variables for mineral nitrogen in the bulk soil
    std::vector<double> soilAmmoniumNitrogen;
    std::vector<double> soilNitrateNitrogen;
    // State variables for litter C and litter N
    std::vector<std::vector<double>> rhizosphereCl;
    std::vector<std::vector<double>> rhizosphereNl;
    // State variables for rhizosphere microbes, labile carbon, and immobile N, and mineral N
    std::vector<std::vector<double>> rhizosphereLiveMicrobialCarbon;
    std::vector<std::vector<double>> rhizosphereDeadMicrobialCarbon;
    std::vector<std::vector<double>> rhizosphereLabileCarbon;
    std::vector<std::vector<double>> rhizosphereMicrobialNitrogen;
    std::vector<std::vector<double>> rhizosphereMineralNitrogen;
    std::vector<std::vector<double>> rhizosphereAmmoniumNitrogen;
    std::vector<std::vector<double>> rhizosphereNitrateNitrogen;
    // State variables for root exudates
    std::vector<std::vector<double>> rootExudateSugarCarbon;
    std::vector<std::vector<double>> rootExudateAminoAcidCarbon;
    std::vector<std::vector<double>> rootExudateAminoAcidNitrogen;
    // State variables for roots
    std::vector<std::vector<double>> maximumRootBiomassCarbon;
    std::vector<double> rootAreaScalar; // rTREES issue... Doesn't appear to be used anywhere...
    std::vector<std::vector<double>> fineRootBiomassCarbon;
    std::vector<std::vector<double>> fineRootBiomassNitrogen;
    std::vector<std::vector<double>> coarseRootBiomassCarbon;
    std::vector<std::vector<double>> coarseRootBiomassNitrogen;
    std::vector<std::vector<double>> rootNSC;
    std::vector<std::vector<double>> rootMineralNitrogen;
    std::vector<std::vector<double>> rootArea;
    ...
    std::vector<double> nitrogenLeaching;
...
public:
    ...
    int Lf_idx; // int* Lf_idx;//rTREES 20221003
    // plant phyllochron tracker
    double phyllo_tracker; // double* phyllo_tracker;//rTREES 20221003
    // plant average SLA
    double SLA_avg;
    // projected leaf area
    double Projected_LeafArea_total;
    // K leaf parameter array
    std::vector<double> Karray; // double* Karray;
    // N leaf parameter array
    std::vector<double> Narray; // double* Narray;
    // r leaf parameter array
    std::vector<double> rarray; // double* rarray;
    ...
    double calcRAR(double thermCD,
        trees_params& treesParams,
        std::vector<double> Karray, std::vector<double> Narray, std::vector<double> rarray,
    ...
    double calc_projGndArea(trees_params& treesParams, // rTREES made by ref 20221020
        std::vector<double> Karray
        // double* Karray
    );
...
private:
    ...
    std::vector<double> SingleLeafBiomassCarbon;
    std::vector<double> SingleLeafBiomassNitrogen;
    std::vector<double> SingleLeafNSC;
    std::vector<double> SingleLeafchloroplastStarch;
    std::vector<double> SingleLeafchloroplastSugar;
    std::vector<double> SingleLeafStoredNitrogen;
    std::vector<double> SingleLeafRubiscoNitrogen;
    std::vector<double> SingleLeafGrowthRespiration;
    // leaf-level state variables unique to single leaves
    std::vector<double> SingleLeafArea;
    std::vector<double> SingleLeafAreaPotential;
    std::vector<double> SingleLeafThermTm;
};
...
public:
    void solveWater(
        ...
        //              double **rflux,
        std::vector<char>& nodeTyp, std::vector<std::vector<std::vector<double>>>& psi,
...
public:
    ...
    void ksolve(trees_params& treesParams,
        double r[][ULAT],
        //		    double pe[][ULAT],
        std::vector<std::vector<double>>& pe,
        ...
        double cc[][2], double kl[][ULAT], double ku[][ULAT],
        //		    double **rflux,

        std::vector<std::vector<double>>& rflux,
        double& modelledKL, double& soilvol,
        // double initial_conductivity_root,double decrement_root, double initial_conducitivty_shoot,double decrement_shoot,
        int& HydraulicModelFailCond);
    ...
    void preSetup(std::vector<double>& md_raw,
        double Md,
...
public:
    ...
    void solvek(trees_params& treesParams,
        double r[][ULAT],
        //		  double pe[][ULAT],
        std::vector<std::vector<double>>& pe,
        double b[][ULAT], double b1[][ULAT], double n[][ULAT],
    ...
    void initialize(trees_params& treesParams,
        double r[MD][ULAT],
        //		     double pe[MD][ULAT],
        std::vector<std::vector<double>>& pe,
        ...
        double al[], double l[], int SoilRhizElements, int smodules, int totmodules,
...
void soil_texture_parameters(trees_params& treesParams,
    int j,
    double& ks,
    double& bubbling_pressure,
    double& pore_size_index,
...
// Infiltration
//
void infiltration(trees_params& treesParams,
    double input,
    State_Store& state,
    std::vector<double>& rootDepth,
    std::vector<double>& thetaSoil,
    std::vector<double>& bypassFlow,
    int rmodules,
    double litter_capacity,
...
void DoHydraulicModel(bool silent,
    ...
    double soilpsiavg[], double nts[], std::vector<double>& evap, double kroot[],
    double axr[], double latr[], double kshoot[], double dslat[], double dsax[], double drlat[],
    double drax[], double l[], double ksat[][MD], double bsat[][MD], double ccsat[][MD],
    double newb[][MD], double newc[][MD], double ip[][ULAT], double b[][ULAT], double b1[][ULAT],
    double n[][ULAT], double n1[][ULAT], double r[][ULAT], double vu[][ULAT], double vl[][ULAT],
...
// not sure where this one is used because verbose is nowhere else used. //MCH
// there is a photosynthesis function in bgc without verbose.
