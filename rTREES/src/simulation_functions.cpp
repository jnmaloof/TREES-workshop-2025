...
struct sim_out simulation_functions(
    ...
    std::vector<double> &thetaSoil, //current state of soil moisture
    std::vector<double> &tempSoil, //current state of soil temperature
    ...
    double Ec_,
    int ModelStatus,
    std::vector<double> LeafPsiInner,
    std::vector<double> KpInner,
    std::vector<int> InnerStep,
    BiogeochemicalCycles& bgc,
    HydraulicModel *hydraulicModel,
    HydraulicModel::outputStruct &ecrit_k_psi,
    HydraulicModel::outputStruct &k_p_e,
    std::vector<double> &ecritOut,
    std::vector<double> &pc,
    std::vector<double> &klpred,
    std::vector<double> &ppredOut,
    std::vector<double> &epredOut,
    std::vector<double> &nodeFail,
    std::vector<char> &nodeTyp,
    std::vector<std::vector<std::vector<double> > > &psi,
    std::vector<std::vector<double> > &rflux,
    // std::vector<char> &nodeTyp,
    // std::vector<std::vector<std::vector<double> > > &psi,
    // std::vector<std::vector<double> > &rflux,
    double soilpsiavg[],
    double nts[],
    std::vector<double> &evap, //rTREES
    double kroot[],
    double axr[],
    double latr[],
    double kshoot[],
...
)
{
  ...
  std::vector<double> rootDepth(ULAT);
  int rmodules;
  ...
  double capFringe;
  std::vector<std::vector<double>> UP_neg(ULAT,std::vector<double>(10));
  std::vector<std::vector<double>>UP_pos(ULAT,std::vector<double>(10));
  std::vector<std::vector<double>> LE_neg(ULAT,std::vector<double>(10));
  std::vector<std::vector<double>>LE_pos(ULAT,std::vector<double>(10));
  double kratio;
  std::vector<double> kratio_vector(ULAT);
  double kratioRoot, nscRatio, rootClassRespRate;
  double CostLoading;
  ...
  double capRise_shallow, capRise_mid, capRise_deep, drain;

  std::vector<double> bypassFlow(ULAT);
  ...
  if (min > 0 && min < 59) min = 30;
  if ((hour+min) == 5.0 && treesParams.useHydraulics == true )
  {
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout <<" \n" << "-------- ";
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << year << ":" << yday;
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << " --------" <<" \n";
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "target LAI = " << treesParams.live_lai;
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "; canopy PLC = " << 100.0*(1.0-k_p_e.latK[1]/ksat[1][1]);
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "%; N status (mol N+ mol organic N) = " << bgc.plantNstatus <<" \n";
  }
  ...
  else if (treesParams.usePhenology == false && treesParams.useLeafModule == false)
  {
    ...
    simOut.leaf_growth = 0.83*delta_nsc/48.0;
    simOut.leaf_growth_respiration = 0.17*delta_nsc /48.0;
  ...
  if (treesParams.useLeafModule == true)
  {
    ...
    if (treesParams.useLeafGamma == true && ts == 1) // update leaf param arrays with sampled values
    {
      ...
      for(i=0; i<500; ++i)
      {
        ...
        //  /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "K" << bgc.Karray[i] <<" \n";
      ...
      for(i=0; i<500; ++i)
      {
        ...
        //  /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "N" << bgc.Narray[i] <<" \n";
      }
      //r parameter
      ...
      for(i=0; i<500; ++i)
      {
        ...
        //  /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "r" << bgc.rarray[i] <<" \n";
      }
    }
    ...
    bgc.phyllo_tracker += delta_thermTime;
    //thermal_time = treesParams.therm_plant;

    // if at first ts, immediately initiate lf
    if (ts == 1)
    {
      bgc.phyllo_tracker = treesParams.phyllochron;
    }

    // checkpoint: enter repro phase?
    if (treesParams.therm_plant >= treesParams.floweringTime)
    {
      // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "Enter Reproductive Phase" <<" \n";
    }
    // else continue in vegetative phase
    else
    {
      if (bgc.phyllo_tracker >= treesParams.phyllochron)
      {
        bgc.Lf_idx++; // initiates new leaf
        bgc.putSingleLeafAreaPotential(bgc.Narray[bgc.Lf_idx-1], bgc.Lf_idx-1 );
        bgc.phyllo_tracker = 0; // resets to 0
      ...
      if (bgc.Lf_idx > 0)
      {
        ...
        for (int k = (bgc.Lf_idx-1); k >= 0; k-- )
        {
          ...
          bgc.updateLeaf(k, delta_thermTime, treesParams,
                         ...
                         bgc.Lf_idx-1, thetaRootTmp);

          // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "leaf " << k << " has potA of " << bgc.getSingleLeafAreaPotential(k) <<" \n";
        }
      }
    }
    ...
    // plant LAI
    treesParams.lai = bgc.calcLAI(bgc.Lf_idx, treesParams, bgc.Karray);
    treesParams.realized_lai=treesParams.lai;
    // plant-level SLA
    if (bgc.Lf_idx > 0 && bgc.getSingleLeafArea(0)> 0)
    {
      //treesParams.SLA = bgc.calcPlantSLA(bgc.Lf_idx[0], treesParams);
      // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "plant level SLA at this ts is " << bgc.calcPlantSLA(bgc.Lf_idx[0], treesParams) << "\n";
      // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "plant level SLA at this ts is " << treesParams.SLA << "\n";
    }

    // plant-level leaf biomass C
    // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "leaf biomass c = " << bgc.getLeafBiomassCarbon() <<" \n";

    // plant-level leaf biomass N
    // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "leaf biomass N = " << bgc.getLeafBiomassN() <<" \n";
  ...
  //DSM July 2024
  //lai = min(treesParams.realized_lai, treesParams.lai);
  lai = treesParams.realized_lai;
  ...
  rmodules = treesParams.rmodules;
  //DSM June 2023 - soil texture in multiple layers
  soil_texture_parameters(treesParams, rmodules+3, ks, bubbling_pressure, pore_size_index, residual);
  ...
  {
    //		Zw = Droot-0.1*rootDepth[rmodules-1];
  ...
  else
  {
    ...
    Zw += capFringe;
    //std::cout << "Zw = " << Zw << "; capFringe = " << capFringe << endl;
  ...
  eff_precip = compute_effective_precipitation(
    state,
    treesParams,
    canopy_store_max,
    precip,
  ...
  for (i = 0; i < rmodules; i++)
  {
    ...
    {
      //DSM June 2023 - soil texture in multiple layers
      soil_texture_parameters(treesParams, i+3, ks, bubbling_pressure, pore_size_index, residual);
      porosity = treesParams.layer_porosity[i+3];
  ...
  if (treesParams.useHydraulics == true)
  {
    water_stress = 500.0*treesParams.E_inc/treesParams.e_at_saturated_kl;
  }
  else
  {
  ...
  canopy_evaporation = compute_canopy_evaporation(
    state,
    canopy_store_max,
    precip,
    canopy_wetness,
  ...
  //
  //DSM 2022 - If soil water content is at residual, then disconnect root
  //Crude - temporary
  //
  /*
   for (i = 0; i < rmodules; i++)
   {
   if (thetaSoil[i] < 1.001*residual)
   if (Ysoil[i] < -10.0)
   {
   psimin_in[i][1] = -10.0;
   }
  ...
  do //coupled plant water balance and carbon assimilation
  {
    ...
    else
    {
      ...
      else if (treesParams.forceRefilling == true && (double) (hour+min) == 3.0)//0300
      {
        callRewet = true;
        callHydraulicModel = 0;
        treesParams.useRefilling = true;
      ...
      if(HydraulicModelFailCond == 1)
      {
        ...
        Rcpp::Rcerr << "*** Hydraulic failure - attempting to recover *** \n";
      }
    }
    ...
    N_avail_rate_plant = bgc.plantNstatus;
    ...
    // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << leafBiomassC << '\t' << leafBiomassN << '\t' << leafBiomassN/MAX(0.000000001,leafArea) << '\t' << Nleaf*Nrubisco*fnr*treesParams.act25*1.0e6 / 60.0<<" \n";
    in.lnc = Nleaf;
    ...
    if (treesParams.useLeafModule == false)
    {
    ...
    if (treesParams.ps_model == 1 || treesParams.ps_model == 2) //C3 photosynthesis
    {
      ...
      {
        A_sun = bgc.photosynthesis(treesParams, Jmax_mult, thetaJ, adj_phiJ_sun, leafpsi,r_sun, in, out);
      ...
      else //use new, experimental C3 model
      {
        ...
        // SC RL 1233 $ 3.2.3 @ 20240630
        // bgc.coupledA3_gc(treesParams, in.co2, Pa, in.irad, adj_phiJ_sun, in.t, gc0, Nmax, gc, A_sun, Ci, Cc, NGsun, NSsun);
        bgc.coupledA3_gc(treesParams, ecrit_k_psi.latK[1], k_p_e.latK[1], ks2[1][1], in.co2, Pa, in.irad, adj_phiJ_sun, in.t, gc0, Nmax, gc, A_sun, Ci, Cc, NGsun, NSsun);
        // EC RL 1234 $ 3.2.3 @ 20240630
    ...
    simOut.J_sun = out.J;
    simOut.Av_sun = out.Av;
    simOut.Aj_sun = out.Aj;
    simOut.betaA_sun = out.betaA;
    simOut.phi2_sun = out.phi2;
    ...
    if (treesParams.ps_model == 1 || treesParams.ps_model == 2) //C3 photosynthesis
    {
      if (treesParams.ps_model == 1) //use the original TREES photosynthesis model (pre-2019)
      {
        A_shd = bgc.photosynthesis(treesParams, Jmax_mult, thetaJ, adj_phiJ_shd, leafpsi, r_sun, in, out);
      ...
      else //use new, experimental C3 model
      {
        ...
        // SC RL 1309 $ 3.2.3 @ 20240630
        // bgc.coupledA3_gc(treesParams, in.co2, Pa, in.irad, adj_phiJ_shd, in.t, gc0, Nmax, gc, A_shd, Ci, Cc, NGshd, NSshd);
        bgc.coupledA3_gc(treesParams, ecrit_k_psi.latK[1], k_p_e.latK[1], ks2[1][1], in.co2, Pa, in.irad, adj_phiJ_shd, in.t, gc0, Nmax, gc, A_shd, Ci, Cc, NGshd, NSshd);
        // EC RL 1310 $ 3.2.3 @ 20240630
    ...
    if (gvc_shd < 0.00001*Gsref0)
    {
      gvc_shd = 0.00001*Gsref0;
    }
    ...
    simOut.Av_shd = out.Av;
    simOut.Aj_shd = out.Aj;
    simOut.betaA_shd = out.betaA;
    simOut.phi2_shd = out.phi2;
    simOut.A = (A_sun*lai_sun + A_shd*lai_shd)/lai;

    //calculate GPP
    A_tot = canopy_cover*(A_sun*lai_sun + A_shd*lai_shd);
    // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << ModelStatus << "; A_tot = " << A_tot << "; Gs = " << simOut.Gs <<" \n";
    ...
    loopcounter++;
    if(
      loopcounter > treesParams.max_iterations||
        r_sun<0.0000001
        )
    {
      Delta = 0.0;
      Converged = true;
      if(treesParams.VERBOSE){
        /*test silent speed*/ Rcpp::Rcout << "!!! Simulation_functions main solver for canopy biochemistry and plant water balance failed to converge !!! \n";
        Rcpp::Rcout<<yday<<"-"<<hour<<"-"<<min<<"\n\n";
    ...
    if ((is > 1) && (PsiCrit >= treesParams.pd_at_sat_kl ||//PSI crit 0 is greater than the negative of pd_at_sat_kl
        std::isnan(leafpsi) ||
        leafpsi > -0.001 ||
        Ec_ > 0.99*Ecrit ||
        HydraulicModelFailCond == 1))
    {
      if( PsiCrit >= treesParams.pd_at_sat_kl ||std::isnan(leafpsi) ||
          leafpsi > -0.001 ||
          HydraulicModelFailCond == 1){
        loopcounter++;
      }
      ///THIS IS HYD FAIL DETECT
      /*test silent speed*/ if(!treesParams.VERBOSE) Rcpp::Rcout << "Hydraulic failure detected! Trying new stomatal conductance. \n\n";
      // if(PsiCrit >= treesParams.pd_at_sat_kl){
      //   // Rcpp::Rcout<<"1 \n";
      //   // Rcpp::Rcout<<"PsiCrit "<<PsiCrit << " treesParams.pd_at_sat_kl "<<treesParams.pd_at_sat_kl<<"\n";
      //   // Rcpp::Rcout<<yday<<"-"<<hour<<"-"<<min<<"\n\n";
      // }
      //   if( std::isnan(leafpsi)){
      //     // Rcpp::Rcout<<"2 \n";
...
// Rcpp::Rcout<<"Delta "<<Delta<<" Ec_max "<<Ec_max<<" rsun "<<r_sun<<"\n";
  ...
  //this would only ever be called if treesParams.max_iterations > 50 and loopcounter>50
  else if(loopcounter > 50)
  {
    ...
    simOut.Leaf_Psi = leafpsi;
    simOut.OutFlag = 2;
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << ">>> !!! FAILED TO CONVERGE !!! \n";
  ...
  for (i = 0; i < rmodules; i++)
  {
    if (treesParams.useHydraulics == true)
    {
      simOut.rhizFlux[i] = k_p_e.rhizFlux[i+2]*ar[i+3];
    }
    else
    {
  ...
  if ((hour+min) == 13.0 && treesParams.useHydraulics == true && treesParams.VERBOSE)
  {
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "Plant hydraulic status (Ks and Weibulls):" <<" \n";
    /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "\tShoot:\t" << "axial Ks=" << ks2[1][0] << ", b=" << b[1][0] << ", c=" << cc[1][0] << "; lateral Ks=" << ks2[1][1] <<	", b=" << b[1][1] << ", c=" << cc[1][1] <<" \n";
    for (i = 0; i < rmodules; i++)
    {
      /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "\tRoot(" << i << ")\t" << "axial Ks=" << ks2[i+3][0] <<	", b=" << b[i+3][0] << ", c=" << cc[i+3][0] << "; lateral Ks=" <<	ks2[i+3][1] << ", b=" << b[i+3][1] << ", c=" << cc[i+3][1] <<" \n";
    }
  }
  ...
  {
    //DSM June 2023 - soil texture in multiple layers
    soil_texture_parameters(treesParams, 3, ks, bubbling_pressure, pore_size_index, residual);
    ks = ks/100.0/3600.0;  //convert from cm/hour to m/s
    porosity = treesParams.layer_porosity[3];
    ...
    // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << litter_store/litter_capacity <<" \n";
    // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << litter_store << '\t' << litter_capacity << '\t' << Rnet_soil << '\t' << t_surface << '\t' << Dsurface << '\t' << gva << '\t' << lai << '\t' << canopy_cover <<" \n";
  ...
  //
  //SOIL WATER BALANCE UPDATES
  //Modified by DSM - May 2023
  //
  //use rhizosphere fluxes to adjust soil water balance in each soil-root layer
  //rhizFluxes mmol s-1  plant-1
  //converted to mmol s-1 m-2 leaf area
  //some adjustment is made as needed to rhizfluxes so ensure water balance
  //Notes: 1) using fscalar results in a tighter water balance
  //       2) using dflux gives a more realistic time seroes of hydraulic redistribution
  //
  double dflux, dfluxRootArea, sumAbsFlux;
  sumRhizFlux = sumAbsFlux = 0.0;
  dfluxRootArea = 0.0;
  for (i = 0; i < rmodules; i++)
  {
    sumRhizFlux += simOut.rhizFlux[i];
    sumAbsFlux += fabs(simOut.rhizFlux[i]);
  }

  //	double fscalar = simOut.Ec*lai/sumRhizFlux;

  //
  //Steady state - seek zero difference between total rhizosphere flux and transpiration
  //
  dflux = sumRhizFlux - simOut.Ec*lai; //mmol m-2 ground area s-1
  for (i = 0; i < rmodules; i++)
  {
    simOut.rhizFlux[i] -= fabs(simOut.rhizFlux[i])/sumAbsFlux * dflux;
    simOut.rhizFlux[i] /= lai; //mmol m-2 leaf s-1

    //		simOut.rhizFlux[i] = simOut.rhizFlux[i] * fscalar / lai;
  ...
  //
  //independently contributing ET from all sources
  //
  ...
  //
  //DSM 2022 - Added to handle extreme dessication state
  //
  //DSM June 2023 - soil texture in multiple layers
  soil_texture_parameters(treesParams, i+3, ks, bubbling_pressure, pore_size_index, residual);
  porosity = treesParams.layer_porosity[i+3];
  if (water_potential(porosity,bubbling_pressure,pore_size_index,residual,thetaSoil[i]) < -10.0)
  {
    simOut.rhizFlux[i] = Rflux[i] = 0.0;
    thetaSoil[i] -= delta_Sr/(rootDepth[i]*mineral_fraction);
  }
  //all layers 2 to j rmodules, transpiration only
  for (i = 1; i < rmodules; i++)
  {
  ...
  //DSM June 2023 - soil texture in multiple layers
  soil_texture_parameters(treesParams, rmodules+2, ks, bubbling_pressure, pore_size_index, residual);
  porosity = treesParams.layer_porosity[rmodules+2];
  ks = ks/100.0/3600.0;  //convert from cm/hour to m/s
  ks = ks*86400.0; // convert from m s-1 to m day-1
  capRise = capRiseScalar*capillary_rise(Zw, bubbling_pressure, pore_size_index, ks)*
    (1.0-(thetaSoil[0]-residual)/(porosity-residual));
  ...
  //Assuming leaf biomass can be approximated by 83% (carbon in cellulose) of NSC use
  ...
  if (t_root >= -50.0)
  {
    ...
    //This function call is used to determine live_lai (or potential LAI)
    //This also computes instantaneous SLA and currently contains scheduling for maize
    ...
    //This is quantified as mean LAI realized during three NH summer months. This needs to
    // be generalized to work in the SH
    //Actual LAI is that which is realized given resources and the forecast potential set from
    // the previous year
    lai_pot = treesParams.live_lai;
    if (yday == 180 && hour == 0 && min == 0)
    {
      //FLAG is this relevant to FP at all

      lai_pot = 0.1*lai_pot + 0.9*treesParams.laiFullCanopyHeight;
    ...
    else if (yday == 270 && hour == 0 && min == 0)
    {
      ...
      //lai_pot = 0.5*lai_pot + 0.5*treesParams.laiFullCanopyHeight;
      state.set_val_at(lai_pot, FORECASTTARGETLAI);
    }//The logic here should be double checked ???
  ...
  /*
   //   Rcpp::Rcout << bgc.getChloroplastStarch() << '\t';
   bgc.putChloroplastStarch(bgc.getChloroplastStarch()-rgrowth/nscRatio);
   //  Rcpp::Rcout << bgc.getChloroplastStarch() <<" \n";
   */

  //Time is forced to 60-min basis above
  if ((hour+min/60.0) >= (12.0-0.5*daylen) && (hour+min/100.0) < (12.5-0.5*daylen))
  {
    if (psn > 0.0)
    {
    ...
    if (treesParams.forceRefilling == true && treesParams.useHydraulics == true)
    {
      ...
      {
        treesParams.useRefilling = false;
        /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "*** Re-filling turned off ***" <<" \n";
      }
      ...
      {
        treesParams.useRefilling = true;
        /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << "*** Re-filling turned on ***" <<" \n";
      }
      if (kratio < 0.2)
      {
        treesParams.useRefilling = false;
  ...
  // /*test silent speed*/ if(treesParams.VERBOSE) Rcpp::Rcout << A_tot << '\t' << simOut.NPP << '\t' << 0.14*rgrowth * 4.6296 <<" \n";
  ...
  //
  // DSM June 2023
  //Soil_Psi is re-computed using a weighted mean based on proportional root areas by root layer
  //For output of Soil_Psi after adjustments
  //
  Soil_Psi = 0.0;
  totArea = 0.0;
  for (i = 0; i < rmodules; i++)
